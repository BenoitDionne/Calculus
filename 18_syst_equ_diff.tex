...
$\DS \frac{c}{ad}$ et ceux de (\ref{LVsystRb}) par
$\DS \frac{b}{a^2}$, nous obtenons
\begin{align*}
...
\quad , \quad
\dydx{\VEC{x}}{t} =
\begin{pmatrix} \DS \dydx{x_1}{t} \\[1em]
\DS \dydx{x_2}{t} \end{pmatrix}
\quad \text{et} \quad
...
\]
pour $t_0\in]a,b[$ et
$\DS \VEC{x}_0 = \begin{pmatrix} x_{0,1} \\ x_{0,2} \end{pmatrix}$
donnés.
...
\quad \text{et} \quad
\dydx{\VEC{x}}{t} = 
\begin{pmatrix} \DS \dydx{x_1}{t} \\[2ex]
\DS \dydx{x_2}{t} \end{pmatrix} \; .
\]
...
$\VEC{x}' = A\VEC{x}$ où
$\DS A = \begin{pmatrix} -1 & 0 \\ 4 & 3 \end{pmatrix}$.
Quelle est la solution de ce système qui satisfait la
condition initiale
$\DS \VEC{x}(0) = \begin{pmatrix} 2 \\ 4 \end{pmatrix}$?
...
$\VEC{x}' = A\VEC{x}$ où
$\DS A = \begin{pmatrix} 0 & -1 \\ 4 & 4 \end{pmatrix}$.
...
$\DS \VEC{v} = \begin{pmatrix} 1 \\ -2 \end{pmatrix}$.
...
$\DS \VEC{u} = \begin{pmatrix} 1 \\ -3 \end{pmatrix}$.
...
Considérons le système d'équations différentielles linéaires d'ordre un
$\VEC{x}' = A\VEC{x}$ où
$\DS A = \begin{pmatrix} 2 & -3 \\ 3 & 2 \end{pmatrix}$.
Quelle est la solution de ce système qui satisfait la condition initiale
$\DS \VEC{x}(0) = \begin{pmatrix} 2 \\ 4 \end{pmatrix}$?
...
$\DS \VEC{u}_1 = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$
et $\DS \VEC{u}_2 = \begin{pmatrix} 0 \\ -1 \end{pmatrix}$.
Toutes les solutions de $\VEC{x}'= A \VEC{x}$ sont de la forme
\begin{align*}
\VEC{x}(t) &
...
$\DS \VEC{f}$ associé au côté droit de (\ref{sediff3}).  Pour
...
chacun de ces points $\DS \begin{pmatrix} x \\ y \end{pmatrix}$.
...
$\DS \begin{pmatrix} 1.5 \\ 4.2 \end{pmatrix}$,
$\DS \begin{pmatrix} 1.5 \\ 3.9 \end{pmatrix}$,
$\DS \begin{pmatrix} 2.5 \\ 3.8 \end{pmatrix}$ et
$\DS \begin{pmatrix} 2.5 \\ 4.0 \end{pmatrix}$
à $t=0$.
...
$\DS \begin{pmatrix} 2 \\ 4 \end{pmatrix}$.  Cela indique 
...
\diff \VEC{f}(\VEC{p}) =
\begin{pmatrix}
\DS \pdydx{f_1}{x_1}(\VEC{p}) &
\DS \pdydx{f_1}{x_2}(\VEC{p}) \\[1em]
\DS \pdydx{f_2}{x_1}(\VEC{p}) &
\DS \pdydx{f_2}{x_2}(\VEC{p})
\end{pmatrix} \; .
\]
...
\phi'(c) =
\begin{pmatrix}
\DS \dydx{\phi_1}{t}(c) \\[2ex] \DS \dydx{\phi_2}{t}(c)
\end{pmatrix}
=
\begin{pmatrix}
\DS 0\\ \DS \dydx{\phi_2}{t}(c)
\end{pmatrix} \; .
\]
\item {\bfseries Les orbites qui coupent un nullcline associé à
...
\phi'(t_r) =
\begin{pmatrix}
\DS \dydx{\phi_1}{t}(t_r) \\[2ex] \DS \dydx{\phi_2}{t}(t_r)
\end{pmatrix}
=
\begin{pmatrix} \DS \dydx{\phi_1}{t}(t_r) \\[2ex] 0\end{pmatrix} \; .
\]
\end{enumerate}
...
$\DS \VEC{p} = \begin{pmatrix} 2 \\ 4 \end{pmatrix}$.
...
$\DS A = \begin{pmatrix} -4 & 1 \\ 1 & 0 \end{pmatrix}$.
...
$\DS \VEC{p}_1 = \begin{pmatrix} -2 \\ -2 \end{pmatrix}$ et
$\DS \VEC{p}_2 = \begin{pmatrix} 0 \\ 0 \end{pmatrix}$.
...
$\begin{pmatrix} x'\\ y' \end{pmatrix} = A
\begin{pmatrix} x\\ y \end{pmatrix}$ où
$\DS A = \begin{pmatrix} -1 & 1 \\ 3 & -1 \end{pmatrix}$.
...
$\DS \VEC{p}_1 = \begin{pmatrix} 0 \\ 0 \end{pmatrix}$,
$\DS \VEC{p}_2 = \begin{pmatrix} 0 \\ 1 \end{pmatrix}$
et
$\DS \VEC{p}_3 = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$.

La linéarisation de (\ref{compexclP}) au point $\VEC{p}_1$ est
$\begin{pmatrix} x' \\ y' \end{pmatrix} = A
\begin{pmatrix} x \\ y \end{pmatrix}$ où
$\DS A = \begin{pmatrix} 1 & 0 \\ 0 & \alpha \end{pmatrix}$.
...
$\begin{pmatrix} x' \\ y' \end{pmatrix} = A
\begin{pmatrix} x \\ y \end{pmatrix}$ où
$\DS A = \begin{pmatrix} -1 & 0 \\-\alpha/3 & -\alpha \end{pmatrix}$.
La matrice $A$ a deux valeurs propres réelles: $-1$ et $-\alpha$.  Puisque
...
\begin{pmatrix} x \\ y \end{pmatrix}$ où \\
$\DS A = \begin{pmatrix} -1 & -2 \\ 0 & 2\alpha/3 \end{pmatrix}$.
...
$\DS \VEC{p}_3 = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$
et
$\DS \VEC{p}_4 = \begin{pmatrix} 3/5 \\ 4/5 \end{pmatrix}$.
...
est $\begin{pmatrix} x' \\ y' \end{pmatrix} = A
\begin{pmatrix} x \\ y \end{pmatrix}$ où
$\DS A = \begin{pmatrix} -1 & -1/2 \\ 0 & 2\alpha/3 \end{pmatrix}$.
Les valeurs propres de $A$ sont $-1$ et $2\alpha/3$.  Puisque
$\alpha>0$, les valeurs propres sont de signes opposés et l'origine
est un col pour le système linéarisé.  Donc
$\VEC{p}_3$ est un col pour le système (\ref{compexclP}). 
...
L'intersection des nullclines associés à $x$ avec ceux associés à $y$
donnent deux points d'équilibre:
$\DS \VEC{p}_1 = \begin{pmatrix} 0 \\ 0 \end{pmatrix}$ et
$\DS \VEC{p}_2 = \begin{pmatrix} 1 \\ 1 \end{pmatrix}$.
...
$\begin{pmatrix} x' \\ y' \end{pmatrix} = A
\begin{pmatrix} x \\ y \end{pmatrix}$ où
$\DS A = \begin{pmatrix} 0 & -1 \\ \alpha & 0 \end{pmatrix}$.
...
$\DS v = \frac{A}{B}(1-u)(C+u)$, les solutions satisfont
...
\[
  F(u,v) = \begin{pmatrix} 
\DS  u \left( 5\left( 1 - u\right) - \frac{10 v}{0.03 + u} \right) \\
\DS  v \left(1 - \frac{v}{u}\right)
  \end{pmatrix} \ .
\]
% \[
%   F(u,v) = \begin{pmatrix} 
% \DS  u \left( A\left( 1 - u\right) - \frac{B v}{C + u} \right) \\
% \DS    v \left(1 - \frac{v}{u}\right)
%   \end{pmatrix} \ .
% \]
Puisque
...
=
\begin{pmatrix}
  \DS \dydx{F_1}{u} & \DS \dydx{F_1}{v} \\[0.7em]
  \DS \dydx{F_2}{u} & \DS \dydx{F_2}{v}
\end{pmatrix} =
\begin{pmatrix}
\DS 5(1-2u) - \frac{0.3 v}{(0.03+u)^2} &
\DS - \frac{10 u}{0.03+ u} \\
\DS \frac{v^2}{u^2} &
\DS 1 - \frac{2v}{u}
\end{pmatrix} \ ,
...
\[
\begin{pmatrix}
\DS \dydx{u}{\tau} \\[0.7em] \DS \dydx{v}{\tau}
\end{pmatrix} =
\begin{pmatrix}
\DS -5 & \DS - \frac{10}{1.03} \\
0 & 1
\end{pmatrix}
\begin{pmatrix}
u \\ v
...
initiale $\DS \VEC{x}(0) = \begin{pmatrix} 0 \\ 1 \end{pmatrix}$
si
\begin{center}
\begin{tabular}{*{2}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS A= \begin{pmatrix} 1 & 12 \\ 3 & 1 \end{pmatrix}$ &
\subQ{b} & $\DS A = \begin{pmatrix} 2 & 3 \\ 4 & 1 \end{pmatrix}$ &
\subQ{c} & $\DS A= \begin{pmatrix} 0 & 1 \\ -1 & -2 \end{pmatrix}$
\end{tabular}
\end{center}
\label{18Q1}
\end{question}
