\nonumsection{Chapter~\ref{ChapFonct} : Fonctions}

\nonumsubsection{Algèbre}

\compileSOL{\SOLUb}{\ref{2Q1}}{
\subQ{a} $\DS (3^4)^{0.5} = 3^2 = 9$

\subQ{b}
$\DS 2^{2^3} \times 2^{2^2} = 2^{2^3 + 2^2} = 2^{8+4} = 2^{12}$

\subQ{c} $\DS \log_3(1) = 0$ car $3^0 = 1$.
...
$\DS x = \frac{x_-+x_+}{2} = -1$; le point milieu entre
...
\]
Si $\DS \frac{x-2}{2x-5} = 1$, nous obtenons
...
\]
La fonction $\DS f(x) = \frac{x-7}{(x+1)(x-3)}$ peut changer de
...
x-7 & - & - & - & + \\
\hline
\rule{0em}{1.5em} \DS f(x) = \frac{x-7}{(x+1)(x-3)} & - & + & - & +
\end{array}
\]
Donc $\DS f(x) > 0$ si $-1<x<3$ ou $x>7$.
...
\]
La fonction $\DS f(x) = -\frac{(x-2)(x+1)}{x+2}$ peut changer de
signe seulement aux points $x$ où $f(x) = 0$ ou $f(x)$ n'est pas
...
\]
La fonction $\DS f(x) = -\frac{(x+9)(x-2)}{(x-3)(x+1)}$
...
\[
  ...
  = \frac{(x-3)(x+2)}{x-1} \ .
\]
La fonction $\DS f(x) = \frac{(x-3)(x+2)}{x-1}$ peut
...
\[
 \frac{x^2-2x - 5}{x-1} < 1 \Leftrightarrow
  ...
  = -\frac{(x-4)(x+1)}{x-1} \ .
\]
La fonction $\DS f(x) = -\frac{(x-4)(x+1)}{x-1}$ peut
changer de signe seulement aux points $x$ où $f(x) = 0$ ou $f(x)$ n'est
...
Par convention, nous écrivons
$\DS y = f^{-1}(x) = \frac{x^2+1}{x^2-1}$.  Le domaine de
...
$\DS \frac{x}{x+1}$ peut prendre toutes les valeurs
...
que $\DS \frac{x_1}{x_1+1} \geq 0$ et
$\DS = \frac{x_2}{x_2+1} \geq 0$ pour obtenir la deuxième
...
\end{align*}
Nous avons utilisé le fait que $\DS \frac{x}{x+1} \geq 0$ pour
obtenir la deuxième équivalence ci-dessus.  Par convention, nous écrivons
$\DS y=h^{-1}(x)= \frac{x^{1/10}}{1 - x^{1/10}}$.  Le
...
\subQ{a} Nous avons $\DS f(x) = f_1(f_2(x))$ où $f_1(y) = 5 y^{-1}$
et $f_2(x) = 1+5^x$.

\subQ{b} Nous avons $\DS h(t) = h_1(h_2(t))$ où $h_1(x) = x^{-4}$
et $h_2(t) = 1-t^2$.
}

\compileSOL{\SOLUb}{\ref{2Q16}}{
Nous avons que $\DS g(x) = g_1(g_2(g_3(x)))$ où $g_1(x) = \cos(x)$,
$g_2(x) = \sqrt{x} = x^{1/2}$ et $g_3(x) = 1+x^2$.
}

\nonumsubsection{Trigonométrie}
...
période et $\DS T = \frac{\pi}{2}$ est la phase.
...
$M = -4$ et la phase est $\DS Z = 2 + \frac{\pi}{6}$.  
Remarque que $\DS Z = 2 - \frac{\pi}{2}$ serait préférable
...
$\DS Z = 0.4$ est la phase.  Remarquons que $Z=-0.1$
...
Nous obtenons le dessin suivant de $\DS \sin(\theta)=\frac{1}{x}$.
\PDFgraph{2_fonctions/triangle3}

Ainsi, $\DS \tan(\theta) = \frac{1}{\sqrt{x^2-1}}$.  La valeur
...
\nonumsubsection{Fonctions exponentielles}
...
Nous avons $\DS S(t) = S_0 10^{0.693 t}$.  Pour répondre à la
...
Donc $\DS p(t) \approx p_0 e^{0.02888\; t}$.  Puisque
$\DS p(12) \approx 500 e^{0.02888\times 12} = 707.1067\ldots$, 
...
$\DS e^{(\text{unités de }\alpha) \times s}$ n'est pas d'unités.  Il
...
Donc $\DS t = \frac{\ln(2)}{1.1} \approx 0.63013380$ s.
...
$\DS Q(t) = Q_0 e^{-0.000122\; t} = \frac{1}{2} Q_0$.
...
Puisque $\DS 200 = \frac{1}{2^3} \times 1600$, nous pouvons
...
Nous cherchons $t$ tel que $\DS 1600 e^{\alpha t} = 437$.  Donc
\begin{align*}
1600 e^{\alpha t} = 437
