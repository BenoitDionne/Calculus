...
\MATHfig{2_fonctions/funct2}{8cm}{Graphe de $\DS h(x) = x^3-2x+1$}
...
fonctions $\DS f(x) = x^2+3$ et $g(x) = 3 x - 2$.  Ainsi,
\begin{align*}
...
\MATHfig{2_fonctions/compog}{8cm}{Graphe de $\DS g(x) = x^2 -1$}
{Graphe de $\DS y= g(x) = x^2 -1$ pour $-2 \leq x \leq 2$}{COMPOG}

\MATHfig{2_fonctions/compohg}{8cm}{Graphe de $y= h(g(x))$}
{Graphe de $\DS y= h(g(x)) = \sqrt{x^2 -1}$ pour $1 \leq |x| \leq 3$}{COMPOHG}

\begin{egg}
Nous pouvons composer les fonctions $\DS g(x) = x^2 - 1$
...
à l'ensemble des $x$ tels que $\DS x^2-1 \geq 0$ pour satisfaire
...
$\DS (g\circ h)(x) = x - 1$ pour tout $x$ mais cela est vrai
...
l'expression $\DS \left(\sqrt{x}\right)^2-1$ n'est pas définie pour $x<0$.
...
Par exemple, si $\DS p(x) = x^2 + 5x$, alors $\Id \circ p = p \circ \Id = p$
...
pour tout $y\in Y$.  Nous dénotons par $\DS f^{-1}$ la fonction $g$ qui est
l'inverse de $f$.
\end{defn}
...
$Y$ est la fonction $\DS f^{-1}$ qui va de l'ensemble $Y$ à l'ensemble $X$
et qui satisfait
\[
...
En effet, si $y=f(x)$ alors $\DS f^{-1}(y) = f^{-1}(f(x)) = x$ et si
$\DS x = f^{-1}(y)$ alors $\DS f(x) = f(f^{-1}(y)) = y$ par définition
...
$Y$.  La fonction inverse $\DS f^{-1}$ est donc la fonction qui, à chaque
capitale dans $Y$, assigne le pays dont elle est la capitale dans $X$.

La fonction $\DS f^{-1}$ est définie à la figure~\ref{INVERSE1} par les
flèches formées de tirets.
\label{FMOINS1}
\end{egg}

\PDFfig{2_fonctions/inverse1}{Définition des fonctions $f$ et $\DS f^{-1}$}
{Définition des fonctions $f$ et $\DS f^{-1}$ de l'exemple~\ref{FMOINS1}}
{INVERSE1}
...
Ainsi, pour $f(x) = x/3 + 1$, nous obtenons $\DS f^{-1}(x) = 3x + 3$.
\end{egg}
...
\begin{egg}
Est-ce que la fonction $\DS f(x) = \frac{1+3x}{5-2x}$ a un
...
{Le graphe de $\DS f(x) = x/3 +1$ et de quelques droites
horizontales}{GRAPH}
...
pour obtenir $x = 3y - 3$.  Donc $\DS x=f^{-1}(y) = 3y-3$.
...
obtenir $\DS y = f^{-1}(x) = 3 x - 3$ pour $x\in \RR$.
\end{egg}
...
Par exemple, soit $\DS f(x) = x^6 + x^2 + 3$.  Il est facile de tracer le
...
Cependant, nous ne pouvons résoudre l'équation $\DS y=x^6+x^2+3$ pour $x$
en fonction de $y$ afin d'obtenir $\DS x = f^{-1}(y)$.  En fait, il a été
...
$\DS f(x) = \frac{1+3x}{5-2x}$ était injective.  Il existe
...
\end{align*}
nous avons $\DS x=f^{-1}(y) = \frac{5y -1}{2y+3}$.  Comme par
...
$\DS y=f^{-1}(x) = \frac{5x -1}{2x+3}$.  La fonction
$\DS f^{-1}$ a comme domaine l'image de la fonction $f$ qui est
$\RR \setminus \{-3/2\}$.
\end{egg}
...
\begin{meth}
Si $f$ est une fonction qui possède une fonction inverse $\DS f^{-1}$,
alors nous obtenons le graphe de $\DS f^{-1}$ en faisant la réflexion du
graphe de $f$ par rapport à la droite $y=x$.
\end{meth}

\PDFfig{2_fonctions/inverse}{Graphe de $\DS f^{-1}$ obtenu par réflexion du
graphe de $f$}{Graphe de $\DS f^{-1}$ obtenu par réflexion du
...
$(f(z),z)$, un point du graphe de $\DS f^{-1}$ car $\DS z = f^{-1}(f(z))$.
...
\begin{egg}
Considérons la fonction $\DS f(x) = x^2 -1$ pour toutes les valeurs
...
Par contre, si nous considèrons $\DS f(x) = x^2-1$ pour $x\geq 0$ seulement,
alors $f$ a un inverse qui est $\DS f^{-1}(x) = \sqrt{x+1}$ pour
$x\geq -1$.  Nous retrouvons à la figure~\ref{INVERSE2} les graphes de
$f$ et $\DS f^{-1}$.

Utilisons la méthode algébrique pour trouver l'inverse de
$\DS f(x) = x^2-1$ pour $x\geq 0$.  Nous additionnons $1$ de chaque
côté de $\DS y = f(x) = x^2 -1$ pour obtenir $\DS y + 1 =  x^2$.
Puis nous prenons la racine carrée des deux côtés de cette dernière
égalité pour obtenir $x = \sqrt{y + 1}$.
Donc $\DS x = f^{-1}(y) = \sqrt{y+1}$ pour $y \geq -1$.
...
$\DS x = \pm \sqrt{y + 1}$.  Or, cette formule ne peut pas
définir une fonction car nous avons deux valeurs pour chaque valeur de $y$.
\end{egg}

\PDFfig{2_fonctions/inverse2}{Graphes de $\DS f(x) = x^2 -1$ et $\DS f^{-1}$}
{Graphes de $\DS f(x) = x^2 -1$ pour $x\geq 0$ et de
$\DS f^{-1}(x) = \sqrt{x+1}$ pour $x\geq -1$.}{INVERSE2}
...
$\DS 90^\circ$) mais comment définir le cosinus ou sinus d'un angle obtus
(plus de $\DS 90^\circ$).
...
Ainsi,  $\DS 360^\circ$ correspond à $2\pi$ radians, la circonférence d'un
cercle de rayon $1$.
...
{\bfseries Fonctions} de $\DS 11^e$ année en Ontario assume
que $x$ est mesuré en degrés.  Alors que la fonction $\cos(x)$
définie dans le cours de {\bfseries Fonctions Avancées} de $\DS 12^e$
...
$\DS \cos_{d}(10) = \cos\left( 10 (\pi/180) \right)$.
...
Puisque $\DS e^{\theta i} = \cos(\theta) + i \sin(\theta)$ où $i$ est le
nombre complexe tel que $\DS i^2=-1$ et $\theta \in \RR$, nous avons
...
l'identité $\DS e^{\theta i} = \cos(\theta) + i \sin(\theta)$ qui n'est
...
% {\DS \frac{\sin(\beta)\sin(\alpha+\beta)}{\cos(\alpha+\beta)}
% + \cos(\beta)} \nonumber \\
...
% \intertext{et}
% \sin(\alpha+\beta) &=
% \frac{\DS \frac{\sin(\beta)}{\cos(\alpha+\beta)}+\sin(\alpha)}
% {\DS \sin(\beta)\frac{\sin(\alpha+\beta)}{\cos(\alpha+\beta)}
% + \cos(\beta)} \nonumber \\
...
\begin{egg}
Quelle est la valeur de $\DS \sin(2\pi/3)$?
...
\]
Puisque $\DS \sin^2(\theta) = 1 - \cos^2(\theta)$, il découle de
...
Il existe une formule pour $\DS \sin^2(\theta)$ qui est
...
Si nous divisons l'identité $\DS \cos^2(\theta) + \sin^2(\theta) = 1$ tour
à tour par $\DS \cos^2(\theta)$ et par $\DS \sin^2(\theta)$, nous obtenons les
...
\DS \frac{\pi}{2} + n \pi \ \text{pour} \ n \in \ZZ \; . \\[0.5em]
...
heures est de $\DS 36.8^\circ$C avec un minimum de $\DS 36.5^\circ$C à 2h00 et
un maximum de $\DS 37.1^\circ$C à 14h00.  Supposons que la
...
Mais! Il y a bien une fonction $\DS \sin^{-1}$ sur les calculatrices.
...
bien injective et nous pouvons définir son inverse $\DS \sin^{-1}$ par la
formule standard
\[
x = \sin^{-1}(y) \qquad \text{si et seulement si} \qquad y = \sin(x)
\]
pour $-\pi/2 \leq x \leq \pi/2$ et $-1 \leq y \leq 1$.  La fonction
$\DS \sin^{-1}$ est appelée l'{\bfseries arcsinus} et est
aussi dénotée $\arcsin$; c'est-à-dire, $\DS \sin^{-1}(x) \equiv \arcsin(x)$.
\end{defn}

En d'autres mots, $\DS x = \sin^{-1}(y)$ est l'angle en radians entre
$-\pi/2$ et $\pi/2$ qu'il faut pour que $\sin(x) = y$.

La définition de $\DS \sin^{-1}$ explique pourquoi cette fonction sur
...
$\DS \sin^{-1}$.  Le lecteur peut vérifier que le graphe de $\DS \sin^{-1}$
est bien obtenu d'une réflexion par rapport à la droite $y=x$ du
graphe de $\sin$.

\MATHfig{2_fonctions/arcsin1}{8cm}{Graphe de $\DS y=\sin^{-1}(x)$}
{Graphe de $\DS y=\sin^{-1}(x)$ pour $-1 \leq x \leq 1$}{ASIN}
...
injective et nous pouvons définir son inverse $\DS \cos^{-1}$ par la formule
standard
\[
x = \cos^{-1}(y) \qquad \text{si et seulement si} \qquad y = \cos(x)
\]
pour $0 \leq x \leq \pi$ et $-1 \leq y \leq 1$.  La fonction
$\DS \cos^{-1}$ est appelée {\bfseries arccosinus} et
est aussi dénotée $\arccos$; c'est-à-dire,
$\DS \cos^{-1}(x) \equiv \arccos(x)$.
\end{defn}
...
injective et nous pouvons définir son inverse $\DS \tan^{-1}$ par la formule
standard
\[
x = \tan^{-1}(y) \qquad \text{si et seulement si} \qquad y = \tan(x)
\]
pour $-\pi/2 < x < \pi/2$ et $y$ réels.  La fonction $\DS tan^{-1}$ est
appelée l'{\bfseries arctangente} et est aussi
dénotée $\arctan$; c'est-à-dire, $\DS \tan^{-1}(x) \equiv \arctan(x)$.
\end{defn}

Le graphe de $\DS \cos^{-1}$ est donné à la figure~\ref{ACOS} et celui de
$\DS \tan^{-1}$ est donné à la figure~\ref{ATAN}.

\MATHfig{2_fonctions/arccos1}{8cm}{Graphe de $\DS y=\cos^{-1}(x)$}
{Graphe de $\DS y=\cos^{-1}(x)$ pour $-1 \leq x \leq 1$}{ACOS}

\MATHfig{2_fonctions/arctan1}{8cm}{Graphe de $\DS y=\tan^{-1}(x)$}
{Graphe de $\DS y=\tan^{-1}(x)$ pour $x$ réels}{ATAN}

\begin{rmk}
Remarquons que $\DS \sin^{-1} \neq \sin^{-1}$

Il faut bien comprendre que $\DS \sin^{-1}$ sur la calculatrice n'est pas
égale à $\DS \left(\sin(x)\right)^{-1} = 1/\sin(x)$.  Par exemple,
\[
...
% \begin{egg}
% Montrons que
% $\DS \sin(\arcsec(x)) = \sqrt{1-\frac{1}{x^2}}$
% pour $|x|>1$.
...
Le but de cette section est de donner un sens à l'expression $\DS b^p$
...
\begin{itemize}
\item L'expression $\DS b^p$ n'est pas définie pour $b=p=0$.
\item $\DS b^p =1$ pour $b>0$ et $p=0$.  Par exemple, $\DS \pi^0 = 1$.
\item Si $b \in \RR$ et $p$ est un entier positif,
\[
b^{p} = \underbrace{b \times b \times \ldots \times b}_{\text{p fois}}
\; .
\]
Par exemple, $\DS \pi^3 = \pi \times \pi \times \pi$.
\item Si $b \in \RR$ et $p = 1/n$ où $n$ est un nombre impair, $\DS y=b^p$
  est le nombre réel tel que $\DS y^n = b$.  Par exemple,
  $\DS (-8)^{1/3} = -2$ car $\DS (-2)^3 = -8$.
\item Si $b \geq 0$ et $p = 1/n$ où $n$ est un nombre pair, $\DS y=b^p$
  est le {\bfseries nombre réel positif} tel que $\DS y^n = b$.  Par exemple,
  $\DS 16^{1/4} = 2$ car $\DS 2^4 = 16$.
\item Si $p = m/n$, nous calculons $\DS \left(b^{1/n}\right)^m$
  s'il est possible de calculer $\DS b^{1/n}$ en premier.
  Il est toujours avantageux de simplifier la fraction $m/n$ avant de
  faire les calculs.  Par exemple, $\DS (-8)^{2/3} = 4$ car
  $\DS (-8)^{1/3} = -2$ et $\DS \left((-8)^{1/3}\right)^2 = (-2)^2 = 4$.
  De même, $\DS 16^{3/4} = 8$ car $\DS 16^{1/4} =2$ et
  $\DS \left(16^{1/4}\right)^3 = 2^3 = 8$.
\item Si $p<0$, nous définissons $\DS b^p$ comme étant le nombre
  $\DS 1/b^{-p}$ si $\DS b^{-p}$ est définie.  Par exemple,
  $\DS \pi^{-3} = 1/\pi^3 = 1/(\pi \times \pi \times \pi)$.
\end{itemize}
}

Aucune définition n'est donnée pour $\DS b^p$ lorsque $b$ est un nombre
réel positif et $p$ est un exposant réel quelconque.  Pour calculer
$\DS 4^2$, il suffit de multiplier $4$ par lui-même.  Pour calculer
$\DS 4^{1/2}$, il suffit de trouver le nombre positif $y$ tel que $\DS y^2=4$;
c'est-à-dire, $y=2$ dans le cas présent.  Mais que devons-nous faire
pour obtenir la valeur de $\DS 4^\pi$?  Nous pouvons utiliser
la définition suivante.

\begin{defn} \label{pr_def_of_exp}
Pour $b >1$ et $p \in \RR$, nous définissons le {\bfseries nombre exponentiel}
$\DS b^p$ comme étant le plus petit nombre réel\footnotemark\ $M$ tel que
$M \geq  b^r$ pour tout nombre rationnel $r\leq p$.

Pour $0<b<1$ et $p \in \RR$, nous définissons le {\bfseries nombre exponentiel}
$\DS b^p$ comme étant le plus grand nombre réel $M$ tel que $\DS M \leq  b^r$
pour tout nombre rationnel $r\leq p$.

Naturellement, si $b=1$, nous obtenons $\DS b^p = 1$ pour tout $p\in \RR$.
\end{defn}
...
Ainsi, $\DS 4^\pi$ est le plus petit nombre réel $M$ tel que $\DS 4^r \leq M$
pour tout nombre rationnel $r\leq \pi$.  De même, $\DS 4^2 = 16$ car
$M=16$ est le plus petit nombre tel que $\DS 4^r \leq M$ pour tout nombre
...
$\DS b^p$ avec $b\in]0,\infty[$ et $p\in \RR$ n'est pas facile à utiliser.
...
exemple, est-ce que cette définition de $\DS b^p$ pour $b>1$ est
équivalente à \lgm $\DS b^p$ est le plus grand nombre réel $M$ tel que
$\DS M \leq  b^r$ pour tout nombre rationnel $r\geq p$\rgm ?  Comme
nous verrons prochainement, cela revient à exiger que les limites à droite et à
gauche de $\DS b^r$ soient égales à $r=p$.  Il y a bien d'autres questions
...
nombre exponentiel $\DS b^p$, $b\in]0,\infty[$ et $p\in \RR$, qui sera
...
Pour l'instant, la valeur d'un nombre exponentiel comme $\DS 4^\pi$
sera estimée
par $\DS 4^r$ pour des valeurs rationnelles de $r$ très près de $\pi$ et
inférieures à $\pi$.  Quelle valeur de $r$, proche de $\pi$, doit-on
utiliser pour obtenir une bonne approximation de $\DS 4^\pi$?  Cette
question ne sera pas abordée.

\begin{defn} \index{Fonction exponentielle}
Si $b$ est un nombre réel positif, la fonction
$f:\RR\rightarrow ]0,\infty[$ définie par $\DS f(x) = b^x$ pour tout $x$
...
Si $f$ est la fonction définie par $\DS f(x) = b^x$ pour tout $x$, nous
...
de $\DS f(x) = b^x$ où $b$, la {\bfseries base}, est un nombre réel
...
Nous donnons à la figure~\ref{LOG1} le graphe des fonctions $\DS f(x) = b^x$
et $\DS f^{-1}(x) = \log_b(x)$ pour $b=3$.  Puisque le domaine de
$\DS f(x) = b^x$ est $\RR$ et son image est $\{ x \in \RR : x > 0 \}$, nous
obtenons bien que le domaine de $\log_b$ est $\{ x \in \RR : x > 0 \}$
et son image est $\RR$.

\MATHfig{2_fonctions/log1}{8cm}{Graphes de $\DS y=3^x$ et de $y=\log_3(x)$}
{Graphes de $\DS y=3^x$ (tirets) et de $y=\log_3(x)$ (ligne pleine)}{LOG1}

Puisque $y=\log_b(x)$ est l'exposant qu'il faut donner à $b$ pour
obtenir $x$ (i.e. pour que $\DS b^y = x$), il est alors simple de vérifier
...
$\DS \log_b(x^p) = p \log_b(x)$.

Si $\DS y = \log_b(x^p)$ alors $\DS b^y = x^p$ par définition du logarithme.
Donc $\DS b^{y/p} = x$.  Une deuxième utilisation de la définition du
logarithme donne $\log_b(x) = y/p$.  Ainsi, si nous résolvons cette
dernière équation pour $y$, nous trouvons $\DS p\log_b(x) = y = \log_b(x^p)$.
\end{egg}

\begin{egg}
Démontrons que $\DS \frac{\log_a(x)}{\log_a(y)} = \log_y(x)$
à partir de la définition du logarithme.

Si $v = \log_a(x)$ et $w = \log_a(y)$, alors $\DS a^v = x$ et $\DS a^w = y$
par définition du logarithme.  Nous obtenons de $\DS a^w = y$ que
$\DS a = y^{1/w}$.  Si nous substituons cette expression pour $a$ dans
$\DS a^v = x$, nous trouvons $\DS x = \left(y^{1/w}\right)^v = y^{v/w}$.  Une
deuxième utilisation de la définition du logarithme donne
$\DS \log_y(x) = \frac{v}{w} = \frac{\log_a(x)}{\log_a(y)}$ par
...
Donc $\DS b^{y/p} = x$.  Une deuxième utilisation de la définition du
logarithme donne $\log_b(x) = y/p$.  Ainsi, si nous résolvons cette
dernière équation pour $y$, nous trouvons
$\DS p\log_b(x) = y = \log_b(x^p)$.
\end{egg}

\begin{egg}
Un modèle fréquemment utilisé pour décrire une population est de la
forme $\DS P(t) = P_0 e^{\alpha t}$ où $P(t)$ est le nombre d'individus au
...
Il faut donc trouver $t$ tel que $\DS P(t) = 2.5 P_0$.
C'est-à-dire, il faut trouver $t$ tel que
$\DS P_0 e^{1.5 t}  = 2.5 P_0$.  Ainsi,
\[
...
$\DS 10^6$ individus après $10$ jours?

Puisque $\DS 10^6 = P(10) = P_0 e^{0.5 \times 10}$, nous
obtenons $\DS P_0 = \frac{10^6}{e^{5}} \approx 6738$ individus.
\end{egg}
...
\subQ{a} & $\DS (3^4)^{0.5}$ &
\subQ{b} & $\DS 2^{2^3} \times 2^{2^2}$ &
\subQ{c} & $\DS \log_3(1)$  \\[0.7em]
\subQ{d} & $\DS \log_{10}(5) + \log_{10}(20)$ &
\subQ{e} & $\DS \log_{10}(500) - \log_{10}(50)$ &
\subQ{f} & $\DS \log_{42.3}(42.3^7)$
\end{tabular}
\end{center}
\label{2Q1}
\end{question}
...
\subQ{a} & $\DS \frac{(x^4 y^{1/4})^{1/2}}{y^{1/2}}$ &
\subQ{b} & $\DS \frac{x+1}{x-2} + \frac{x+1}{x-3} +
 \frac{-9x+21}{x^2-5x+6}$
\end{tabular}
\end{center}
\label{2Q2}
...
$\DS \frac{x^3 + 2x^2 +x+3}{x+2}$ sous la forme
...
\begin{tabular}{*{2}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS x^2+x-6$ &
\subQ{b} & $\DS 3x^2 - 5 x -2$ &
\subQ{c} & $\DS x^{3/2} + x^{1/2}-12 x^{-1/2}$
\end{tabular}
\end{center}
\label{2Q4}
\end{question}
...
\subQ{a} Quelles sont les racines du polynôme $\DS x^2 + 2x -2$?\\
\subQ{b} Tracez la parabole $\DS y=x^2+2x-2$.  N'oubliez
...
\begin{question}
Quelle valeur devez-vous donner à $c$ dans le polynôme $\DS 4x^2 -12x + c$
...
\begin{center}
\begin{tabular}{*{2}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS 7\times 5^{3x} = 21$ &
\subQ{b} & $\DS 4\times 3^{-2x+1} = 7\time 3^{3x}$ &
\subQ{c} & $\DS \log_{10}(100^x) = 3$
\end{tabular}
\end{center}
\label{2Q7}
\end{question}
...
\begin{center}
\begin{tabular}{*{1}{l@{\hspace{0.5em}}l@{\hspace{6em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS \frac{2x}{x+1} = \frac{2x-1}{x}$ &
\subQ{b} & $\DS \frac{1}{x+1} + \frac{3}{x-1} = 4$ \\[1em]
\subQ{c} & $\ln(x-3)+\ln(x-5) = \ln(2x-6)$ &
\subQ{d} & $|x-2| = 5$ \\[1em]
\subQ{e} & $|x-2| = |2x-5|$ &
\subQ{f} & $\DS |x^2 - 4| = 4$ \\[1em]
...
\begin{center}
\begin{tabular}{*{2}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS 5 -\frac{6}{x} < 4$ &
\subQ{b} & $\DS x^2 -4x + 3 > 0$ &
\subQ{c} & $\DS \frac{2}{x+1} >\frac{1}{x-3}$ \\[1em]
\subQ{d} & $\DS \frac{x^2}{x+2}  < 1$ &
\subQ{e} & $\DS \frac{x}{x-3}  < \frac{-6}{x+1}$  &
\subQ{f} & $|x-2| < 5$ \\[1em]
\subQ{g} & $\DS |x^2-2x - 5| < |x-1|$ & & & &
\end{tabular}
\end{center}
\label{2Q10}
\end{question}
...
\begin{question}
Quel est l'image de la fonction $\DS f(x) = 3 + 2x-x^2$?
\label{2Q11}
\end{question}
...
\begin{tabular}{*{2}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS g(x) =5^{x^3}$ &
\subQ{b} & $\DS f(x) = \sqrt{ \frac{x+1}{x-1} }$ &
\subQ{c} & $\DS h(x) = \left(\frac{x}{x+1}\right)^{10}$ \\
\subQ{d} & $\DS y=f(x) = \left(\frac{2x+7}{3}\right)^{-1/2}$ &
& & &
\end{tabular}
\end{center}
\label{2Q13}
...
$\DS f(x) = \sqrt{x^2-1}$ et $\DS g(x) = \sqrt{x^2+1}$?
\label{2Q14}
\end{question}

\begin{question}
Écrivez les fonctions suivantes comme la composition de fonctions
simples.  Pour nous, les fonctions simples sont $\DS f(x)=x^a$,
$\DS a^x$, $\log_b(x)$, ..., et le produit et la somme de telles
fonctions.
\begin{center}
\begin{tabular}{*{1}{l@{\hspace{0.5em}}l@{\hspace{6em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS f(x) = \frac{5}{1+5^x}$ &
\subQ{b} & $\DS h(t) = (1-t^2)^{-4}$
\end{tabular}
\end{center}
\label{2Q15}
\end{question}
...
Écrivez la fonction $\DS g(x) = \cos(\sqrt{1+x^2})$ comme la
composition de fonctions simples.  Pour nous, les fonctions simples
sont $\DS f(x)=x^a$, $\DS a^x$, $\log_b(x)$, $\cos(x)$, ..., et le produit et
...
\subQ{a} $\DS \cos(\theta-\pi) = - \cos(\theta)$.

\subQ{b} $\DS \cos(2\theta) = \cos^2(\theta) - \sin^2(\theta)$.
\label{2Q17}
\end{question}
...
\subQ{c} & $\DS f(x) = -3 + 5\sin\left(\frac{\pi}{2}(2-x)\right)$ &
\end{tabular}
\end{center}
\label{2Q20}
\end{question}
...
\subQ{a} & $\DS h(z) = 1 + 5 \cos\left(\frac{\pi z}{2} -
  \frac{3\pi}{2}\right)$ &
...
\begin{question}[\eng \life]
Si $\DS \sin(\theta)=\frac{1}{x}$, quelles sont les valeurs
...
\begin{question}[\eng \life]
Montrez que
$\DS \tan(\arcsin(x)) = \frac{x}{\sqrt{1-x^2}}$ pour $-1<x<1$.
\label{2Q25}
\end{question}
...
l'autre est exponentielle (de la forme $\DS p\, b^x$).  Trouvez une formule
...
donné par la formule $\DS S(t) = S_0 10^{\alpha t}$ où le
...
$\DS p(t) = p_0 e^{\alpha t}$ où $p_0$ est le nombre initial
...
donné par l'expression $\DS V(t) = V_0 e^{\alpha t}$.  Quelles sont les
...
$\DS Q(t) = Q_0 e^{-0.000122\; t}$ où $Q_0$ est la quantité de
...
\begin{question}[\eng \life]
Tracez le graphe de la fonction $\DS g(t) = e^t \cos(2\pi t)$ pour $0<t<3$.
