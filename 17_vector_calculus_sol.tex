\nonumsection{Chapter~\ref{chapCalcVect} : Calcul vectoriel}

\nonumsubsection{Intégrale le long d'une courbe}
...
\nonumsubsection{Rotationnel et divergence}
...
$\DS f(x,y,z) = e^x\cos(z) + \frac{y^2}{2} + h(z)$.
Finalement,
\[
...
$\DS f(x,y,z) = e^x\cos(z) + \frac{y^2}{2} + C$.
}
...
$\DS \pdydx{F_2}{x} = \pdydx{F_1}{y}$ pour démontrer que $F$
...
\]
nous avons bien que $\DS \pdydx{F_2}{x} = \pdydx{F_1}{y}$ et
...
(exemple~\ref{rotFzeroD2}), il suffit de vérifier que
$\DS \pdydx{F_2}{x} = \pdydx{F_1}{y}$ pour démontrer que $F$ est
...
\]
nous avons bien que $\DS \pdydx{F_2}{x} = \pdydx{F_1}{y}$ et $F$
...
$\DS \curl F = \VEC{0}$ pour démontrer que $F$ est
conservateur.  Or,
\begin{align*}
\curl\,F & = \left( \pdydx{F_3}{y} - \pdydx{F_2}{z} \right) \ii +
\left( \pdydx{F_1}{z} - \pdydx{F_3}{x} \right) \jj +
...
\end{align*}
pour une certaine fonction $h:\RR \to \RR$.  Nous avons donc
$\DS f(x,y,z) = x^2yz  +\sin(yz) + h(z)$.  Finalement,
\begin{align*}
  \pdydx{f}{z} = F_3(x,y,z) &\Rightarrow
...
\nonumsubsection{Intégrale sur une surface}
...
= -32 \pi
\end{align*}
car $\DS \int_0^{2\pi} \cos(\theta) \dx{\theta} = 0$
et $\DS \int_0^{2\pi} \cos^3(\theta) \dx{\theta} = 0$.
}
...
$\DS 8 \pi (2 - \sqrt{2}) + \frac{2\pi}{3}(5\sqrt{5}-1)$.
}
...
\end{align*}
Le vecteur $\DS \VEC{q}$ est un vecteur normal à la surface $S$
...
\nonumsubsection{Théorèmes de Stokes et de Green}
...
$\DS \frac{\partial(x,y)}{\partial(r,\theta)} = r $ et
...
$\DS \int_C F \cdot \dx{\VEC{s}}$ en utilisant la
...
\nonumsubsection{Théorème de la divergence}
...
\left| \det \begin{pmatrix}
  \DS \pdydx{x}{\theta} & \DS \pdydx{x}{r}
  & \DS \pdydx{x}{z} \\[0.8em]
  \DS \pdydx{y}{\theta} & \DS \pdydx{y}{r}
  & \DS \pdydx{y}{z} \\[0.8em]
  \DS \pdydx{z}{\theta} & \DS \pdydx{z}{r}
  & \DS \pdydx{z}{z}
\end{pmatrix} \right|
= 
\left| \det \begin{pmatrix}
...
= r
\]
et $\DS \diV F(x,y,z) = x^2 + y^2$, nous obtenons
\begin{align*}
\iiint_E \diV  F \dx{V} &=
