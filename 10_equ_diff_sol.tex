\nonumsection{Chapter~\ref{chap_equ_diff} : Équations différentielles}

\nonumsubsection{Introduction}

\compileSOL{\SOLUb}{\ref{10Q1}}{
La fonction $\DS y(t) = t^2\sin(t)$ est une solution car
\[
...
$C=77$.  Ainsi $\DS L(t) = -72 e^{-0.09t} + 77$ et
...
$C=4.5$.  Ainsi, $\DS P(t) = -2.5 e^{-2 t} + 4.5$ et
...
Ainsi, $\DS A(t) = 174.6 (t-1981)^3 + 13,400$ et
...
jour.  Notons que
$\DS \int_5^9 x'(t) \dx{t} = x(9)-x(5)$ est la variation du
...
\subQ{b} Puisque $\DS \dydx{x}{t}(t) = v(t)$, nous obtenons
$\DS \dydx{x}{t}(t) = e^t$.
...
\subQ{b} Puisque $\DS \dydx{x}{t}(t) = v(t)$, nous avons
$\DS \dydx{x}{t}(t) = 0.1 + 0.2 e^{-t}$.
...
- 5 \, t^{-1} + C \ .
\]
Si $p(1)=12$, alors $\DS \frac{5}{3} - 5 + C = 12$
et ainsi $\DS C=\frac{46}{3}$.  Nous avons tracé le graphe de
$\DS p(t)=\frac{5}{3} \, t^3 - 5 \, t^{-1} + \frac{46}{3}$
et celui de $\DS p'(t) = 5t^2 + \frac{5}{t^2}$ ci-dessous.
...
\]
Si $M(2)= 8$, alors
$\DS 8 = \frac{2^4}{4} + \frac{2^{-2}}{-2} + C = \frac{31}{8} + C$
et ainsi $\DS C = \frac{33}{8}$.  Nous obtenons la solution
\[
...
La solution cherchée est donc $\DS p(t) = -\frac{t^4}{16} + 5$.
...
\subQ{a} Nous avons $\DS P(t) = \int \frac{5}{1+2t} \dx{t}$.
Si $u=1+2t$, alors $\DS \dx{u} = 2 \dx{t}$ et
\begin{align*}
...
$\DS P(t) = \frac{5}{2} \ln|1+2t| + 5$.  Nous avons que
$P(t) \to \infty$ lorsque $t\to \infty$.

\subQ{b} Nous avons $\DS P(t) = \int \frac{5}{(1+t)^2} \dx{t}$.
...
\]
et
$\DS W(2) = -\frac{34}{27} e^{-6} + \frac{10}{27} \approx 0.36725$.
...
\]
et
$\DS M(1) = -\left(\frac{1}{2}+ \frac{1}{2} + \frac{3}{4}
...
$\DS x'(t) = \int x''(t) \dx{t} = -10.5 t + C$ où
...
$\DS x(t) = -\frac{10.5}{2}\; t^2 + 5 t + 100$.
...
$\DS -\frac{10.5}{2}\; t^2 + 5 t + 100$ a une seul racine
positive qui est
$\DS t_s =
...
$\DS x(t) = - \frac{1}{2} + \frac{3}{2} e^{2t}$ est une
solution car
\[
...
\nonumsubsection{Équations différentielles séparables}
...
$\DS k = \frac{1}{30} \, \ln(1/2) \approx -0.023104906$.

Si la quantité initiale de Césium $137$ est $q_0 = 50$ mg, il restera
$\DS q(50) = 50 e^{50k} \approx 15.749013$ mg 
après $50$ ans.
}

\compileSOL{\SOLUb}{\ref{10Q26}}{
\subQ{a} Puisque $\DS y'(t) = 2 y(t)$, nous obtenons
\[
...
$\DS -\frac{2}{5} = \frac{-2}{3+2D}$.
Nous obtenons $D=1$ et donc $\DS x(t) = \frac{-2}{3 t^2 + 2}$.
...
$\DS \dydx{h}{t} = 1000 -h$.  Nous obtenons donc
\[
h(t) = 1000 - D e^{-t}
\]
avec $D$ une constante réelle.  Pour satisfaire $h(0)=500$, nous devons
choisir $D$ tel que $\DS 500 = 1000 - D$; c'est-à-dire,
...
\]
Notez que
$\DS g(t) = \frac{1}{2} \left( -1 - \sqrt{ 1+ 20t}\right)$
...
\ln|y| = \arctan(x) + C \ .
\]
Donc $\DS y = D e^{\arctan(x)}$ où $D = \pm e^C$ est une
...
dernière équation, nous obtenons $\DS 1 = \frac{1}{-6C}$ car
$\DS y(0) = 1$.  Donc $C=-1/6$ et
$\DS y = \frac{1}{-2\arctan(3x) + 1}$.
}
...
$\DS y = \frac{ E e^{4t} - 2}{4}$, nous obtenons
\[
...
\]
La solution est donc $\DS c(t) = C-(C-c_0) e^{-kA t/V}$.
}
...
t < 10^{4/5} \ .
\]
Notons que $\DS \lim_{t\to (10^{4/5})^-} y(t) = \infty$.
...
La solution générale de $\DS \dydx{x}{t} = 2x-1$ est donc
\[
...
différentielle $\DS \dydx{T}{t} = K ( M- T)$ où la constante $M$
...
$\DS \dydx{T}{t} = K(M - T)$ où $M$ est la température à
...
\]
car $t\geq D$.  Donc
$\DS \dydx{y}{t} = -2 \sqrt{y}$ pour $t\leq D$.
...
\nonumsubsection{Équations différentielles autonomes}
...
$\DS \theta = \frac{\pi}{2} + n \pi$ pour $n\in \ZZ$.
...
La dérivée de $f$ est $\DS f'(C) = -1 + \frac{2}{(2+C)^2}$.
...
$\DS \dydx{C}{t} = F(C)$ où $F(C) = 5 - C$.  Les points d'équilibre
...
$P'(t) = K P(t) (1-P(t)/M)$ est $\DS P(t) = \frac{M}{1+Ce^{-Kt}}$
...
$\DS 1000 = P(0) = \frac{1800}{1+C}$ (qui provient de
$P(0) = 1000$) et
$\DS 1100 = P(1) = \frac{1800}{1+Ce^{-K}}$ (qui provient de 
$P(1) = 1100$).
...
\subQ{a} Nous avons une équation différentielle de la forme
$\DS \dydx{p}{t} = f(p)$ où $f(p) = 2\,p(1-d-p)- p$.  Les
...
\end{align*}
où $\DS D=\pm e^{-0.01 C}$.   Notons que $D=0$ est aussi
...
$\DS Q'= 1.25 - 0.02 Q$ avec la condition
...
\nonumsubsection{Méthode d'Euler}
...
$\DS f(x,y) = \frac{3x^2}{2y}$.  Soit $y$ la solution de
...
$\DS w_{i+1} = w_i + h\,f(x_i,w_i) = w_i + \frac{0.3 x_i^2}{2w_i}$
pour $i=0$, $1$. $2$, \ldots, $9$.  Ainsi,
\begin{align*}
...
résoudre l'équation différentielle $\DS y'=\frac{3x^2}{2y}$
...
cherchée est $\DS y(x) = \sqrt{x^3 + 1}$.
...
$\DS y(1) = \sqrt{2} \approx 1.414213562373095$,
