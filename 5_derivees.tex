\chapter[Dérivée]{Dérivée}\label{ChapDerivee}
...
fonction $\DS g(t) = 50 \; e^{-2t}$ représente le
...
que le taux de variation moyen $\DS \frac{f(b)-f(a)}{b-a}$
...
= 0.693147180559945\ldots
\]
Est-ce que $\DS \frac{p'(t)}{p(t)} = 0.693147180559945\ldots$
...
$\DS \left\{x_n\right\}_{n=1}^\infty$ qui tendent vers $0$.
Dans le premier tableau, les valeurs
$\DS \frac{g(x_n)-g(0)}{x_n-0}$
approchent $1$ alors que dans le second tableau, les valeurs
$\DS \frac{g(x_n)-g(0)}{x_n-0}$ approchent $-1$.
\[
\begin{array}{l|l|l}
\hline
\rule[-1em]{0ex}{2.7em} n & \DS x_n = \frac{1}{n} &
\DS \frac{g(x_n)-g(0)}{x_n-0} \\
\hline
1 & 1 & 1 \\
2 & 1/2 & 1 \\
...
$\DS \frac{g(x_n)-g(0)}{x_n-0}$ approche $M$ pour toute
suite $x_n$ qui approche $0$ lorsque $n \to \infty$.  Donc
$\DS \lim_{x\rightarrow 0} \frac{g(x)-g(0)}{x-0}$ n'existe 
...
la notation $\DS \dydx{f}{x}$ pour désigner $f'$.
\end{defn}
...
l'aide du rapport $\DS \frac{f(x+h)-f(x)}{h}$ où
$h = 0.00001$.
\[
\begin{array}{l|l|l}
\hline
x & \rule{0em}{1.7em}
\DS f'(x) \approx \frac{f(x+0.00001)-f(x)}{0.00001}
& |2x - f'(x)| \\[0.8em]
\hline
-1 & -2 & 0 \\
...
valeurs de $x$ à l'aide du rapport $\DS \frac{f(x+h)-f(x)}{h}$ où
$h = 0.00001$.
...
\cos(x)$ \`a l'aide du rapport $\DS \frac{f(x+h)-f(x)}{h}$
...
l'approximation $\DS \frac{f(x+h)-f(x)}{h}$ de $f'(x)$.
...
&= - 1 \times 0 = 0
\end{align*}
car $\DS \frac{\sin(h)}{\cos(h)+1}$ est une fonction
...
\begin{align*}
\dfdx{\tan(x)}{x} &= \frac{\left(\DS \dfdx{\sin(x)}{x}\right) \cos(x)
- \sin(x) \left(\DS \dfdx{\cos(x)}{x}\right)}{\cos^2(x)} \\
...
$x$ & $\DS f'(x) \approx \frac{f(x+h)-f(x)}{h}$ & $f'(x)/3^x$ &
\qquad \qquad &
$x$ & $\DS f'(x) \approx \frac{f(x+h)-f(x)}{h}$ & $f'(x)/2^x$
\\[0.8em]
\cline{1-3} \cline{5-7}
...
$\DS \frac{f(x+h)-f(x)}{h}$ où $h = 0.00001$\ .\label{TAB_32_EXP}}
\end{table}
...
$\DS \frac{f(x+h)-f(x)}{h}$ où $h = 0.00001$.
...
$\DS C_b \equiv \lim_{h\rightarrow 0} (b^h-1)/h$ est une
...
$\DS \lim_{h\rightarrow 0} \frac{b^h-1}{h}$ existait pour
...
La suite $\DS \left\{h_n\right\}_{n=1}^\infty$ converge vers
$0$.  Donc $\DS \frac{3^{h_n}-1}{h_n}$ converge vers $C_3$
lorsque $n \to \infty$.  Nous obtenons les résultats suivants.
\[
\begin{array}{c|c|l}
\hline
\rule[-0.4em]{0ex}{1.5em} n & h_n = 1/n^2 &
\rule{0em}{1.7em} \DS \frac{3^{h_n}-1}{h_n} \\[0.8em]
\hline
1 & 1 & 2 \\
...
\end{array}
\]
Nous avons donc que $\DS \frac{3^{h_n}-1}{h_n}$ approche la constante
...
$\DS \lim_{h\rightarrow 0} \frac{e^h-1}{h} = 1$ et le nombre
...
de $b$ telle que $\DS \lim_{h\rightarrow 0} \frac{b^h-1}{h} = 1$
...
$\DS g(x) = \frac{(x^4+2)e^{2x}}{(x^2+3x)\sqrt{x^2+3}}$.
...
$\DS \frac{f(x+h)-f(x)}{h}$ où
$h = 0.00001$, nous obtenons les résultats suivants.
\[
\begin{array}{l|l|l}
\hline
\rule[-0.7em]{0ex}{2.5em} x &
f'(x) \approx \DS \frac{f(x+0.00001)-f(x)}{0.00001}
 & |-x^{-2} - f'(x) | \\[0.8em]
\hline
...
il faut se rappeler que $\DS x^\alpha = e^{\alpha \ln(x)}$
...
\begin{egg}
Si $f(x) = x^{-1/2}$, alors
$\DS f'(x) = -\frac{1}{2} x^{-3/2} = \frac{-1}{2 x^{3/2}}$
pour $x>0$.
\end{egg}
...
& $\DS f(x) = \frac{1}{2x+5}$ & \subQ{c} & $f(x) = x^2+x$ \\[0.9em]
\subQ{d} & $\DS f(x) = \frac{x}{3x-1}$ &
\subQ{e} & $\DS f(x) = \frac{1}{x(x+1)}$ & &
\end{tabular}
\end{center}
\label{5Q18}
\end{question}
...
pente $\DS \frac{H(1)-H(-1)}{2}$ de la sécante entre les points
...
\subQ{a} & $\DS f(x) = x^{1/5}$ &
\subQ{b} & $\DS h(t) = x^{1/e}$ &
\subQ{c} & $\DS g(z) = 3z^3 + 2z^2$ \\[0.7em]
\subQ{d} & $\DS p(z) = (1+3z)^2(1+2z)^3$ &
\subQ{e} & $\DS f(x) = (2x+1)^3$ &
\subQ{f} & $\DS h(x) = (x^2-5)^{135/2}$ \\
\subQ{g} & $\DS G(x) = \frac{(1+x)(2+x)}{(3+x)}$ &
\subQ{h} & $\DS f(t) = \frac{\sqrt{t}}{1+\sqrt{t}}$ &
\subQ{i} & $\DS f(x) = \log_2(1-3x)$ \\[0.7em]
\subQ{j} & $\DS f(y) = (5y-3)^7(y^2-1)$ &
\subQ{k} & $\DS h(t) = \frac{1+t}{2-t}$ &
\subQ{l} & $\DS g(z) = \frac{1+z^2}{1+2z^3}$ \\[0.7em]
\subQ{m} & $\DS f(x) = \frac{\ln(2x+ e^x)}{2x+e^x}$ &
\subQ{n} & $\DS h(x) = \frac{(1+3x)^2}{(1+2x)^3}$ &
\subQ{o} & $\DS f(x) = e^{-7x}$ \\[1em]
\subQ{p} & $\DS g(z) = \left(1 + \frac{2}{1+z}\right)^7$ &
\subQ{q} & $\DS f(t) = (1+3t)^{33}$ &
\subQ{r} & $\DS h(x) = \ln|\ln(x)|$ \\[1em]
\subQ{s} & $\DS g(t) = \ln\left(\frac{t^2}{(t-2)^3}\right)$ &
\subQ{t} & $\DS f(x) = x^{\ln(x)}$ &
\subQ{u} & $\DS f(x) = x^{x^2}$ \\[1em]
\subQ{v} & $\DS g(t) = \ln\left(\frac{t^7(t^2-5)^8}{(t-2)^5}\right)$ &
 &  & &
\end{tabular}
\end{center}
\label{5Q26}
...
\subQ{a} & $\DS h(\theta) =\sec(\theta)\tan(\theta)$ &
\subQ{b} & $\DS G(t) = 1 + 2
\cos\left( \frac{2\pi(t-3)}{5} \right)$ \\[1em]
\subQ{c} & $\DS f(x) = \ln(4+\sin(x))$ &
\subQ{d} & $\DS g(\theta) = \sqrt{2\theta\, \sin(\theta)}$ \\
\subQ{e} & $\DS F(\theta) = \tan^2(\sin(\theta))$ &
\subQ{f} & $\DS g(x) = x \arctan(x^2)$\\
\subQ{g} & $\DS h(\theta) = \sin(\theta)\cos(\theta)$ &
\subQ{h} & $\DS h(x) = 3 + \cos(2x-1)$ \\
\subQ{i} & $\DS g(z) = e^{\cos(z)}$ &
\subQ{j} & $\DS f(\theta) = \sec(\theta)$ \\
\subQ{k} & $\DS f(x)=e^{\sin(x^{100}+1)}$ &
\subQ{l} & $\DS g(x) = x^{\sin(x)}$ \\
\subQ{m} & $\DS f(t) = \ln\left(\frac{2t-\cos(6t)}{t}\right)$  &
         &
\end{tabular}
\end{center}
\label{5Q27}
...
\begin{question}
Calculez la dérivée de $\DS f(x) = \frac{1}{1+e^x}$ de deux
façons:
...
\begin{question}
Calculez la dérivée de $\DS f(x) = \ln(7x)$ de deux façons:
...
\begin{question}
Calculez la dérivée de $\DS f(x) = 7^x$ de deux façons:
...
\begin{question}[\eng \life]
Calculez la dérivée de $\DS f(\theta) = \cos(2\theta)$ de deux
façons:
...
\subQ{c} & Si $\DS h(x) = \frac{f(x)}{g(x)}$, calculez $h'(1)$. & 
