\nonumsection{Chapter~\ref{chapter_integr} : Intégrale}

\nonumsubsection{Intégrales indéfinies}

\compileSOL{\SOLUb}{\ref{7Q1}}{
\subQ{a} Puisque
$\DS \int x^\alpha \dx{x} = \frac{x^{\alpha+1}}{\alpha+1} + C$
pour $\alpha \neq -1$, nous avons
$\DS \int -x^{-2} \dx{x} = x^{-1} + C$ pour $\alpha = -2$.
...
\subQ{e}
Puisque $\DS \dfdx{\left(2^x\right)}{x} = 2^x \ln(2)$,
nous avons
\[
...
\subQ{d}
Si $\DS y = 1+\frac{t}{3}$, alors
$\DS \dx{y} = \frac{1}{3} \dx{t}$ et
\begin{align*}
...
par parties.  Nous avons $\DS \frac{\ln(x)}{x} = f(x)g'(x)$ où
...
\subQ{p}
Si $\DS t = u^5$, alors $\dx{t} = 5u^4 \dx{u}$ et 
\[
...
5 \int \frac{u}{1+u^2} \dx{u} \ .
\end{align*}
Pour évaluer $\DS \int \frac{u}{1+u^2} \dx{u}$, nous posons
$v= 1+u^2$.  Ainsi,
\[
...
$\DS \ln(x) = f(x)g'(x)$ où $f(x) = \ln(x)$ et
...
$\DS \int x e^x\dx{x}$.  Nous avons $x e^x = f(x) g'(x)$ pour
...
$\DS (y-2)\ln(y) = f(y)g'(y)$ où $f(y)=\ln(y)$ et
$g'(y)=(y-2)$.  Donc $\DS f'(y)= \frac{1}{y}$,
$\DS g(y) = \frac{(y-2)^2}{2}$ et
\begin{align*}
...
\]
une primitive de $\DS \int e^x\sin(x) \dx{x}$.  Ainsi,
\[
\int e^x\sin(x) \dx{x}
...
$\DS \frac{x^3-x-2}{x^2-4} = x + \frac{3x-2}{x^2-4}$.
...
\]
Pour évaluer $\DS \int \frac{x}{x^2+3} \dx{x}$, nous posons
...
\nonumsubsection{Définition de l'intégrale définie}
...
La distance parcourue est $\DS \int_0^{10} v(t) \dx{t}$.  C'est
...
& \approx -0.856759 \ .
\end{align*}
Donc $\DS \int_1^6 (\sqrt{x}-2) \dx{x} \approx -0.856759$.
}
...
\subQ{b} Puisque
$\DS G_N \geq \int_0^8 v(t) \dx{t} \geq D_N$ pour une
...
$\DS \sum_{j=1}^n f(x_j) \Delta x$ où $f(x)=x^3$, $n=5$,
...
Puisque $\DS f(z) = f(0) + \int_0^z f'(x) \dx{x}
...
\]
car $\DS \int_0^{10} f'(x) \dx{x} = 0$ puisque l'aire entre
...
Il ne faut pas oublier que $\DS \int_5^{10} f'(x) \dx{x}$ est
...
$\DS \int_z^1 F(x) \dx{x}$ pour $z < 1$ est négative car c'est
moins l'aire enter l'axe des $x$ et la courbe $y=F(x)$.
\PDFgraph{7_integrales/integral3_sol1}
Donc $\DS \int_0^1 F(x) \dx{x} = -\frac{5}{3}$ et
\[
...
$\DS \int_3^z F(x) \dx{x}$ est l'aire entre la courbe
$y=F(x)$ et l'axe des $x$.  Donc
$\DS \int_3^6 F(x) \dx{x} = 3$ et
\[
...
$\DS \int_0^{6} F(x) \dx{x} = 0$ et
$\DS f(6) = f(0) + \int_0^6 F(x) \dx{x} = f(0)$.
...
$\DS \int_1^z G(x)\dx{x} = - \int_z^1 G(x) \dx{x}$
où $\DS \int_z^1 G(x)\dx{x} > 0$ car $G(x)>0$.
...
Pour $2\leq z \leq 5$. Nous avons $\DS \int_2^z G(x)\dx{x} <0$ car
...
$\DS \int_5^z G(x) \dx{x}$ est l'aire entre la courbe
$y=G(x)$ et l'axe des $x$.  Donc
$\DS \int_5^6 F(x) \dx{x} \approx 0.5$ et
\[
...
\nonumsubsection{Intégrales définies}
...
formules suivantes:
$\DS \int_1^3 = \int_1^2 + \int_2^3$ (que nous avons utilisé
ci-dessus), $\DS \int_1^2 = \int_1^3 - \int_2^3$ et
$\DS \int_2^3 = \int_1^3 - \int_1^2$.
}
...
\subQ{e}
Si $u=1+2x^3$, alors $\DS \dx{u} = 6 x^2 \dx{x}$, $u=1$
...
$\DS \int_0^{2\pi} \cos(x)\dx{x} = 0$ car l'aire de la
...
\subQ{e}
Puisque $\DS \frac{x^3+x+1}{x^2+4} = x + \frac{-3x+1}{x^2+4}$,
nous avons
\[
\int_0^2 \frac{x^3+x+1}{x^2+4} = \int_0^1 x \dx{x}
-3 \int_0^2 \frac{x}{x^2+4} \dx{x} + \int_0^2 \frac{1}{x^2+ 4} \dx{x} \ .
\]
Pour évaluer $\DS \int_0^2 \frac{x}{x^2+4}$, nous posons
...
avons que $\DS \int_{-\pi/2}^{\pi/2} \sin(x) \dx{x} = 0$.  Puisque 
...
Donc $\DS \int_{2}^{5}\cos(2\pi(x-2)) \dx{x} = 0$.
...
$\DS \int_{-\pi}^{\pi} x^{16} \sin(2x) \dx{x} = 0$.
}
...
$\DS h_1(z) = \int_2^z \arctan(t) \dx{t}$ et $h_2(x)= 1/x$.
...
\nonumsubsection{Intégrales impropres}
...
$\DS \int_0^\infty \frac{1}{\sqrt[3]{x}} \dx{x} $ diverge.
...
\end{align*}
Donc l'intégrale $\DS \int_0^3\frac{2}{(x-2)^{4/3}}\dx{x}$ diverge.

\subQ{j} Considérons $\DS \int_0^N \frac{x}{1+x^4} \dx{x}$.
...
= \frac{2}{3}
\end{align*}
car $\DS \lim_{q\to \infty} \frac{1}{(1+3q)^{1/2}} = 0$.
L'intégrale converge.

\subQ{l}
Puisque $\DS \frac{1}{x^2+x-6} = \frac{1}{(x+3)(x-2)}$,
...
$\DS \frac{1}{x^2+x-6} = \frac{1}{5(x-2)} - \frac{1}{5(x+3)}$.
Ainsi,
\begin{align*}
...
\ln(3)\right) = -\infty
\end{align*}
car $\DS \lim_{q\to 2^-} \ln|q+3| = \ln(5)$ et
$\DS \lim_{q\to 2^-} \ln|q-2| = \lim_{q\to 0^+} \ln(q) = -\infty$.
Donc $\DS \int_0^2 \frac{1}{x^2+x-6} \dx{x}$ diverge.  Ce qui
implique que $\DS \int_0^4 \frac{1}{x^2+x-6} \dx{x}$ diverge.
L'intégrale converge.
...
$\DS \int_0^1 \frac{1}{x^{1/3}+x^3} \dx{x}$ converge
car $\DS \int_0^1 \frac{1}{x^{1/3}} \dx{x}$ converge d'après la
...
$\DS \int_1^\infty \frac{1}{x^{1/3}+x^3} \dx{x}$ converge
car $\DS \int_1^\infty \frac{1}{x^3} \dx{x}$ converge d'après la
...
$\DS \int_1^\infty \frac{x}{x^2+x^{4/3}} \dx{x}$ diverge
car $\DS \int_1^\infty \frac{1}{2x}\dx{x}$ diverge d'après
...
comparaison que $\DS \int_0^1 \frac{1}{x^2\, e^x} \dx{x}$
diverge car $\DS \int_0^1 \frac{1}{e\, x^2} \dx{x}
...
\]
Il découle du test de comparaison que
$\DS \int_1^\infty \frac{1}{\sqrt{x}+ e^x} \dx{x}$
converge car $\DS \int_1^\infty \frac{1}{e^x} \dx{x}$ converge.
De plus,
\[
...
\subQ{c}
Puisque
$\DS 0\leq \frac{\sin^2 x}{x^3 + 2} \leq \frac{1}{x^3}$ pour
$x \geq 1$, il découle du test de comparaison que
$\DS \int_1^\infty \frac{\sin^2 x}{x^3 + 2} \dx{x}$
converge car $\DS \int_1^\infty \frac{1}{x^3} \dx{x}$ converge
...
\subQ{d} Puisque
$\DS 0 \leq \frac{1+\cos^2x}{x^2} \leq \frac{2}{x^2}$ pour
$x\neq 0$, il découle du test de comparaison que
$\DS \int_1^\infty\frac{1+\cos^2x}{x^2}\dx{x}$ converge car
$\DS \int_1^\infty\frac{2}{x^2} \dx{x}$ converge d'après la
...
pour $0 \leq x \leq 1$.
Il découle du test de comparaison que
$\DS \int_0^1 \frac{2\sqrt{x}+1}{x^4+3x} \dx{x}$
diverge car $\DS \int_0^1 \frac{1}{4x}$ diverge d'après la
proposition~\ref{impr_comp2} avec $p=1 \geq 1$.

\subQ{g} Pour $x$ très grand, nous avons que
$\DS \frac{2\sqrt{x}+3}{x^2+2x}$ se \lgm comporte\rgm\ comme
$\DS \frac{2\sqrt{x}}{x^2} = \frac{2}{x^{3/2}}$.
Puisque l'intégrale 
$\DS \int_1^{\infty} \frac{1}{x^{3/2}}\dx{x}$ converge grâce
à la proposition~\ref{impr_comp1} avec $p = 3/2 >1$, il est tentant de
d'essayer de démontrer que
$\DS \frac{2\sqrt{x}+3}{x^2+2x} < \frac{A}{x^p}$ pour une
constante $A$ et $p > 1$.
...
Il découle du test de comparaison que
$\DS \int_1^\infty \frac{2\sqrt{x}+3}{x^2+2x} \dx{x}$
converge car $\DS \int_1^\infty \frac{5}{x^{3/2}}\dx{x}$
converge.  De plus,
\begin{align*}
