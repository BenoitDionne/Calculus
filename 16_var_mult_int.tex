...
dénotés $\DS \iint_D f(x,y) \dx{A}$, est le nombre $I$ tel
...
$\DS \iint_D f(x,y) \dx{A}$ en choisissant
...
\]
(figure~\ref{VOLUME1}).
Nous avons alors que $\DS \int_D f(x,y) \dx{A}$ est le volume du
solide définie par
\[
...
notation $\DS \iiint_D f(\VEC{x}) \dx{V}$ pour
...
un rectangle $D$,
\begin{enumerate}
\item Nous évaluons l'intégrale $\DS \int_c^d f(x,y) \dx{y}$ en
assumant que $x$ est constant pour obtenir une fonction $g(x)$ et par la
suite nous évaluons l'intégrale $\DS \int_a^b g(x) \dx{x}$.
\item Nous évaluons l'intégrale $\DS \int_a^b f(x,y) \dx{x}$ en
assumant que $y$ est constant pour obtenir une fonction de $h(y)$ et par la
suite nous évaluons l'intégrale $\DS \int_c^d h(y) \dx{y}$.
\end{enumerate}
...
\begin{egg}
Évaluons l'intégrale double $\DS \iint_R (x-2y) \dx{A}$
...
\begin{egg}
Évaluons l'intégrale double $\DS \iint_R 3x \dx{A}$
...
\begin{egg}
Évaluons l'intégrale double $\DS \iint_R (y^2+ x) \dx{A}$ où
...
(e.g. $\DS \int_a^b = - \int_b^a$) alors que l'intégrale de
...
\diff g(\VEC{x}) =
\begin{pmatrix}
  \DS \pdydx{y_1}{x_1} & \DS \pdydx{y_1}{x_2} \\[0.8em]
  \DS \pdydx{y_2}{x_1} & \DS \pdydx{y_2}{x_2}
\end{pmatrix}
\]
Le {\bfseries Jacobian}\index{Jacobian} de la fonction $g$, dénoté
$\DS \frac{\partial(y_1,y_2)}{\partial(x_1,x_2)}$, est
définie par
\[
...
\diff g(\VEC{x}) =
\begin{pmatrix}
  \DS \pdydx{y_1}{x_1} & \DS \pdydx{y_1}{x_2}
  & \DS \pdydx{y_1}{x_3} \\[0.8em]
  \DS \pdydx{y_2}{x_1} & \DS \pdydx{y_2}{x_2}
  & \DS \pdydx{y_2}{x_3} \\[0.8em]
  \DS \pdydx{y_3}{x_1} & \DS \pdydx{y_3}{x_2}
  & \DS \pdydx{y_3}{x_3}
\end{pmatrix} \ .
\]
...
$\DS \int_{-a}^a \int_0^{\sqrt{a^2-y^2}} (x^2 + y^2)^{3/2}
\dx{x} \dx{y}$.
...
\left| \frac{\partial(x,y)}{\partial(r,\theta)} \right| = 
  ...
  \DS \pdydx{x}{r} & \DS \pdydx{x}{\theta}\\[0.8em]
...
\left| \det \begin{pmatrix}
  \DS \pdydx{x}{r} & \DS \pdydx{x}{\theta}
  & \DS \pdydx{x}{z} \\[0.8em]
  \DS \pdydx{y}{r} & \DS \pdydx{y}{\theta}
  & \DS \pdydx{y}{z} \\[0.8em]
  \DS \pdydx{z}{r} & \DS \pdydx{z}{\theta}
  & \DS \pdydx{z}{z}
\end{pmatrix} \right|
= \left| \det \begin{pmatrix}
...
\left| \det \begin{pmatrix}
\DS \pdydx{x}{r} & \DS \pdydx{x}{\theta}
& \DS \pdydx{x}{\phi} \\[0.8em]
\DS \pdydx{y}{r} & \DS \pdydx{y}{\theta}
& \DS \pdydx{y}{\phi} \\[0.8em]
\DS \pdydx{z}{r} & \DS \pdydx{z}{\theta}
& \DS \pdydx{z}{\phi}
\end{pmatrix} \right| \\
&= \left| \det \begin{pmatrix}
...
\subQ{a} $\DS \iint_R \sin(2x+y) \dx{A}$ où
$R = \{ (x,y) : 0 \leq x \leq \pi/4 \ , \ 0 \leq y \leq \pi/2 \}$.

\subQ{b} $\DS \iint_R \frac{2+y}{3+x} \dx{A}$ où
$R = \{ (x,y) : -1 \leq x \leq 1 \ , \ -1 \leq y \leq 2 \}$.
\label{16Q1}
\end{question}

\begin{question}
Évaluez $\DS \iint_R (2x+3y)^2 \dx{A}$ où $R$ est la région
...
\subQ{a} & $\DS \int_0^1 \int_{y^{1/3}}^1 \sqrt{x^4+1} \dx{x}\dx{y}$
& \subQ{b} &
$\DS \int_0^{\pi^{1/4}} \int_{y^2}^{\pi^{1/2}} y\sin(x^2)
\dx{x}\dx{y}$
\end{tabular}
\end{center}
\label{16Q3}
...
\subQ{a} $\DS \int_0^4 \int_{-\sqrt{x}}^{\sqrt{x}} f(x,y) \dx{y}\dx{x}
+ \int_4^9\int_{-\sqrt{x}}^{6-x} f(x,y) \dx{y} \dx{x}$ \\
\subQ{b} $\DS \int_{-6}^{-3} \int_{-\sqrt{6-x}}^{\sqrt{6-x}}
...
\begin{question}
Évaluez l'intégrale $\DS \iint_S xy \dx{V}$ où $S$ est le tétraèdre
...
\begin{question}
Évaluez l'intégrale $\DS \iint_S xz \dx{V}$ où $S$ est le tétraèdre
...
$\DS \int_0^2 \int_0^{y^3} \int_0^{y^2} f(x,y,z) \dx{z}\dx{x}\dx{y}$
...
\begin{question}
Récrivez l'intégrale
$\DS \int_{0}^{4} \int_{\sqrt{x}}^{2} \int_{0}^{2-y}
...
\begin{question}
Soit l'intégrale $\DS \int_0^2 \int_{-\sqrt{x^2+y^2}}^0
...
\subQ{b} & $\DS \int_0^1 \int_x^{\sqrt{2-x^2}}
...
$\DS \rho(x,y,z) = \frac{1}{\sqrt{x^2+y^2+z^2}}$ g/cm$^3$.
...
$x^2+y^2+z^2 =25$ et le cône $\DS \frac{16}{9} z^2 = x^2 + y^2$ pour
