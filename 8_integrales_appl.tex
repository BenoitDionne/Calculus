...
$\DS A = \int_{-4}^0 |f(x)-g(x)|\dx{x}$, il faudrait utiliser
...
Riemann pour $\DS \int_a^b f(x) \dx{x}$.  Ainsi, si $k$ tend vers
...
des cercles de rayon $\DS r= \frac{3}{5} (7+x)$.  Donc
$\DS A(x) = \frac{9\pi}{25} (7+x)^2$.
...
est de $2r = 5+x$ m de long.  Donc $\DS A(x) = 8(5+x)$.
...
$\DS 2r = 2\sqrt{2^2-(x+7)^2}$ m de long.  Donc
$\DS A(x) = 18\sqrt{2^2-(x+7)^2}$.
...
$\DS -7 \int_{-2}^{2} \sqrt{2^2-u^2} \dx{u}$, nous posons
...
Nous trouvons que
$\DS F = 19,600\times 29 \times \frac{\pi}{2} = 284,200\pi$
Newtons.
...
$\DS \VEC{v}_j = \begin{pmatrix} p_j \\ q_j \end{pmatrix}$ et
...
$\DS \left(\frac{M_y}{m},\frac{M_x}{m}\right) = (0, 2/5)$.
\end{egg}
...
\]
car $\DS \left( \frac{\pi R^4 P}{8\eta L}\right) \Delta t$
...
Posons $\DS t_i = \frac{i}{2}$ pour $i=0$, $1$, \ldots,
$10$.  Nous avons
\[
...
$\DS D = \frac{\pi P R^4}{8\eta L}$ que
\[
...
années est
$\DS M_0 = M_n \left(1 + \frac{I}{100k}\right)^{-k\,n}$.
...
$\DS 250,000\,e^{-4/100}$, la valeur présente du montant de
\$$250,000$ qui vous sera donné dans deux ans est
$\DS 250,000\,\left(e^{-4/100}\right)^2$, etc.
...
$\DS e^{I\,(T-t_i^\ast)/100} \,R(t_i^\ast)\, \Delta t$ car ces
...
$\DS e^{-I\,t_i^\ast/100} \,R(t_i^\ast)\, \Delta t$.
...
$\DS \sum_{n=1}^\infty a_n$ converge à l'aide d'une intégrale 
impropre.

\begin{theorem}[Le test de l'intégrale] \index{Test de l'intégrale}
Soit $f:[1,\infty[\rightarrow [0,\infty[$ une fonction continue et
décroissante.  Si $\DS \sum_{n=1}^\infty a_n$ est une série dont
les termes sont donnés par $a_n = f(n)$ pour tout $n$, alors la série
$\DS \sum_{n=1}^\infty a_n$ converge si et seulement si l'intégrale
impropre $\DS \int_1^\infty f(x)\dx{x}$ converge.
\end{theorem}
...
Posons $\DS T_n = \int_1^n f(x)\dx{x}$ et
$\DS S_n = \sum_{k=1}^n a_k$.  Les sommes $S_n$ sont les
...
$\DS \sum_{k=2}^6 a_k = S_6 - a_1$ alors que la région en gris
dans le dessin à droite dans la figure~\ref{int_TEST} représente la
valeur de la somme $\DS \sum_{k=1}^6 a_k = S_6$.

\subQ{A} Supposons que $\DS \int_1^\infty f(x)\dx{x}$ diverge. Cela
implique que $\DS \lim_{n\rightarrow \infty} T_n = +\infty$.
...
\]
et nous concluons que la séries $\DS \sum_{n=1}^\infty a_n$ diverge.
Cela implique que si la série $\DS \sum_{n=1}^\infty a_n$
converge alors l'intégrale impropre
$\DS \int_1^\infty f(x)\dx{x}$ converge.

\subQ{B} Supposons que $\DS \int_1^\infty f(x)\dx{x}$ converge.
Cela implique que la limite $\DS \lim_{n\rightarrow \infty} T_n$
...
$\DS \{S_n\}_{n=1}^\infty$ est une suite croissante (car
...
$\DS \{S_n\}_{n=1}^\infty$ converge.  C'est à dire que
$\DS \sum_{n=1}^\infty a_n$ converge.
\end{proof}

\begin{rmk}
Dans la cas où la série $\DS \sum_{n=1}^\infty a_n$
converge, et donc l'intégrale impropre
$\DS \int_1^\infty f(x)\dx{x}$ converge, nous pouvons utiliser
...
$\DS \{S_n\}_{n=0}^\infty$ est une suite croissante car les
termes de la série $\DS \sum_{n=1}^\infty a_n$ sont
positifs,  Donc
...
Si $p \leq 0$, alors la suite $\DS \{n^{-p}\}_{n=1}^\infty$
ne converge pas vers zéro.  Donc, par le théorème~\ref{NCforCofS},
la séries $\DS \sum_1^\infty \frac{1}{x^p} \dx{x}$
diverge.
...
$\DS \int_1^\infty \frac{1}{x^p} \dx{x}$ converge si seulement et
seulement si $p>1$.
...
\begin{egg}
Déterminons si la série $\DS \sum_{n=1}^\infty n 5^{-n}$
converge ou diverge.

Cette série est de la forme $\DS \sum_{n=1}^\infty a_n$ où
...
Considérons l'intégrale $\DS \int_1^q x 5^{-x}\dx{x}$.
...
$\DS \lim_{q\rightarrow \infty}q 5^{-q} = 
\lim_{q\rightarrow \infty} \frac{q}{5^q} = 0$.  De plus,
$\DS \lim_{q\rightarrow \infty} 5^{-q} =
\lim_{q\rightarrow \infty} \frac{1}{5^q} = 0$.  Donc
\[
\lim_{q\rightarrow \infty} \int_1^q x 5^{-x} \dx{x}
= \frac{1}{5\ln(5)} + \frac{1}{5(\ln(5))^2} \; .
\]
Puisque $\DS \int_1^q x 5^{-n}\dx{x}$ converge, la série
$\DS \sum_{n=1}^\infty n 5^{-n}$ converge.
\end{egg}

\begin{egg}
Déterminons si la série
$\DS \sum_{n=2}^\infty \frac{1}{n\ln{n}}$ converge ou diverge.

Cette série est de la forme $\DS \sum_{n=2}^\infty a_n$ où
$\DS a_n = f(n)$ pour
$\DS f(x) = \frac{1}{x\ln(x)}$.
...
Considérons l'intégrale $\DS \int_2^q \frac{1}{x\ln(x)} \dx{x}$.
Cette dernière intégrale se calcul à l'aide de la méthode de
substitution.  Si $u = \ln(x)$, alors
$\DS \dx{u} = \frac{1}{x} \dx{x}$, $u = \ln(q)$ lorsque
...
$\DS \sum_{n=2}^\infty \frac{1}{n\ln{n}}$ diverge.
\end{egg}
...
$\DS y=\frac{e^{2x}}{2}$ pour $0\leq x \leq 1$.\\
\subQ{d} La région bornée par la droite $\DS y = \frac{3}{2} + x$
et la courbe $\DS y = \frac{x^2}{2}$.\\
...
$\DS y = f(x)=\frac{x}{1+x}$ et la droite 
$\DS y=g(x)=\frac{x}{2}$.\\
\subQ{h} La région bornée par les courbes
$\DS y = (x-2)^2$ et $\DS y= 10 - x^2$.
\label{8Q1}
\end{question}
...
$\DS r(t) = \frac{1000}{(2+3t)^{3/4}}$ bactéries/heure.
...
\subQ{a} & $\DS \sum_{n=1}^\infty n e^{-2n^2}$ &
\subQ{b} & $\DS \sum_{n=3}^\infty \frac{1}{n(\ln(n))^2}$
\end{tabular}
\end{center}
\label{8Q36}
\end{question}
...
$\DS \sum_{n=3}^{\infty} \frac{\ln n}{n^k}$ converge.
\label{8Q37}
\end{question}

\begin{question}
Démontrez que la séries $\DS \sum_{n=1}^\infty \frac{1}{n^{8/3}}$
converge et trouvez une petite valeur $N$ pour que la somme partielle
$\DS S_N = \sum_{n=1}^N \frac{1}{n^{8/3}}$ soit une
approximation de la valeur de la série
$\DS S = \sum_{n=1}^\infty \frac{1}{n^{8/3}}$
