...
\begin{defn} \index{Dérivée partielle en un point}
...
\begin{defn} \index{Dérivée partielle}
...
$\DS \pdydx{f}{x_1}(x_1,x_2)$, cette dérivée partielle est la
dérivée d'une fonction d'une seule variable, $x_1$, qui dépend d'un
paramètre, $x_2$.  De même, $\DS \pdydx{f}{x_2}(x_1,x_2)$ est
...
\begin{defn} \index{Dérivée partielle!d'ordre supérieure}
...
\begin{defn} \index{Fonction de classe $C^k$}
...
La notation $\DS \dydx{z}{t}$ peut porter à confusion car
...
$\DS \frac{x^2}{3} + \frac{y^2}{9} + \frac{z^2}{9} = 1$.
...
\begin{defn} \index{Représentation paramétrique d'une surface}
...
\item $\DS \pdydx{\rho}{u_1}(u_1,u_2)$ et
$\DS \pdydx{\rho}{u_2}(u_1,u_2)$ sont deux vecteurs
...
\begin{defn} \index{Dérivée directionnelle}
...
$\DS \pdydx{f}{\VEC{u}}(\VEC{a})$, est définie par
\[
\pdydx{f}{\VEC{u}}(\VEC{a}) =
...
gradient de $f$ à $\DS \VEC{a} = \begin{pmatrix}
a_1 & a_2 & \cdots & a_n \end{pmatrix}^\top$ comme la matrice de dimension
\nm{1}{n}
\[
\nabla f(\VEC{a}) =
\begin{pmatrix}\DS \pdydx{f}{x_1}(\VEC{a}) &
\DS \pdydx{f}{x_2}(\VEC{a}) & \ldots &
\DS \pdydx{f}{x_n}(\VEC{a}) \end{pmatrix} \; .
\]
Puisque $\DS \VEC{u} = \begin{pmatrix} u_1 & u_2 & \cdots &
...
$\DS F(x,y,z) = \frac{x^2}{3} + \frac{y^2}{9} + \frac{z^2}{9} = 1$.
...
$\DS \dydx{f}{\VEC{u}}(\VEC{a})$ est lorsque
...
\begin{defn} \index{Ensemble convexe}
...
$\DS \pdfdxn{}{x_j}{\alpha_j}$ lorsque $\alpha_j = 0$.
...
$\DS \pdfdxn{f}{\VEC{x}}{\alpha} = \pdfdxn{}{x_1}{2} \left(
...
\begin{theorem}[Théorème de Taylor] \index{Théorème de Taylor}
...
\diff f(\VEC{x}) &=
\begin{pmatrix}
\DS \pdydx{f}{x_1}(\VEC{x}) &
\DS \pdydx{f}{x_2}(\VEC{x}) &
\cdots & \DS \pdydx{f}{x_n}(\VEC{x})
\end{pmatrix}
\intertext{et}
  H(\VEC{x}) &=
\begin{pmatrix}
\DS \pdydxn{f}{x_1}{2}(\VEC{x}) &
\DS \pdydxnm{f}{x_2}{x_1}{2}{}{}(\VEC{x}) &
\cdots & \DS \pdydxnm{f}{x_n}{x_1}{2}{}{}(\VEC{x}) \\[0.8em]
\DS \pdydxnm{f}{x_1}{x_2}{2}{}{}(\VEC{x}) &
\DS \pdydxn{f}{x_2}{2}(\VEC{x}) & 
\cdots & \DS \pdydxnm{f}{x_n}{x_2}{2}{}{}(\VEC{x}) \\
\vdots & \vdots & \ddots & \vdots \\
\DS \pdydxnm{f}{x_1}{x_n}{2}{}{}(\VEC{x}) &
\DS \pdydxnm{f}{x_2}{x_n}{2}{}{}(\VEC{x}) & 
\cdots & \DS \pdydxn{f}{x_n}{2}(\VEC{x})
\end{pmatrix}
\end{align*}
...
$\DS \pdydx{f}{x_1}(\VEC{p}) = \pdydx{f}{x_2}(\VEC{p}) = 0$;
c'est-à-dire, $\nabla f(\VEC{p}) = \VEC{0}$.
...
$\DS \frac{1}{2} (\VEC{x} - \VEC{p})^\top H(\VEC{p}) 
...
Donc, si $\DS \frac{1}{2} (\VEC{x} - \VEC{p})^\top H(\VEC{p})
...
\[
H(\VEC{p}) = \begin{pmatrix} \DS \pdydxn{f}{x_1}{2}(\VEC{p})
& \DS \pdydxnm{f}{x_2}{x_1}{2}{}{}(\VEC{p}) \\[0.9em]
\DS \pdydxnm{f}{x_1}{x_2}{2}{}{}(\VEC{p}) &
\DS \pdydxn{f}{x_2}{2}(\VEC{p})
\end{pmatrix}
\]
...
H(x,y) = \begin{pmatrix}
\DS \pdydxn{f}{x}{2}(x,y) &
\DS \pdydxnm{f}{y}{x}{2}{}{}(x,y) \\[0.9em]
\DS \pdydxnm{f}{x}{y}{2}{}{}(x,y) &
\DS \pdydxn{f}{y}{2}(x,y)
\end{pmatrix}
= \begin{pmatrix}
  6 x & -6 \\
...
\begin{meth}[Méthode de Lagrange] \index{Méthode de Lagrange}
...
$\DS \pdydx{f_i}{x_j}(\VEC{c})$ pour $1\leq i \leq m$ et
$1\leq j \leq n$ existent.  De plus,
\[
\diff f(\VEC{c}) =
\begin{pmatrix}
\DS \pdydx{f_1}{x_1}(\VEC{c}) &
\DS \pdydx{f_1}{x_2}(\VEC{c}) & \ldots &
\DS \pdydx{f_1}{x_n}(\VEC{c}) \\[1em]
\DS \pdydx{f_2}{x_1}(\VEC{c}) &
\DS \pdydx{f_2}{x_2}(\VEC{c}) & \ldots &
\DS \pdydx{f_2}{x_n}(\VEC{c}) \\[1em]
\vdots & \vdots & \ddots & \vdots & \\[1em]
\DS \pdydx{f_m}{x_1}(\VEC{c}) &
\DS \pdydx{f_m}{x_2}(\VEC{c}) & \ldots &
\DS \pdydx{f_m}{x_n}(\VEC{c})
\end{pmatrix} \; .
\]
\end{theorem}
...
\diff f(\VEC{x}) =
\begin{pmatrix}
\DS \pdydx{f_1}{x_1}(\VEC{x}) &
\DS \pdydx{f_1}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{f_1}{x_n}(\VEC{x}) \\[1em]
\DS \pdydx{f_2}{x_1}(\VEC{x}) &
\DS \pdydx{f_2}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{f_2}{x_n}(\VEC{x}) \\[1em]
\vdots & \vdots & \ddots & \vdots & \\[1em]
\DS \pdydx{f_m}{x_1}(\VEC{x}) &
\DS \pdydx{f_m}{x_2}(\VEC{x}) & \ldots &
\DS \pdydx{f_m}{x_n}(\VEC{x})
\end{pmatrix} \; .
\]
\end{prop}
...
\diff f(\VEC{c}) &=
\begin{pmatrix}
\DS \frac{-2x}{(x^2+y^2)^2}\bigg|_{(x,y)=(1,2)} &
\DS \frac{-2y}{(x^2+y^2)^2}\bigg|_{(x,y)=(1,2)} \\[1em]
...
\diff f(\VEC{c}) =
\begin{pmatrix}
\DS \pdydx{f}{x_1}(\VEC{c}) &
\DS \pdydx{f}{x_2}(\VEC{c}) & \ldots &
\DS \pdydx{f}{x_n}(\VEC{c})
\end{pmatrix}
= \nabla f(\VEC{c})
\]
...
\diff \phi(c) =
\begin{pmatrix}
\DS \dydx{\phi_1}{t}(c) \\[2ex]
\DS \dydx{\phi_2}{t}(c) \\ \vdots \\
\DS \dydx{\phi_m}{t}(c)
\end{pmatrix}
= \phi'(c)
\]
...
\subQ{a} & $\DS f(x,y,z,t) = xy^2z^3t^4$ &
\subQ{b} & $\DS f(x,y) = \frac{2x+y}{x-y}$ &
\subQ{c} & $\DS h(x,y) = f(x)g(y)$ \\[0.8em]
\subQ{d} & $\DS h(x,y) = f(xy)$ &
\subQ{e} & $\DS h(x,y) = f\left(\frac{x}{y}\right)$ & &
\end{tabular}
\end{center}
\label{15Q1}
\end{question}
...
$\DS \frac{\partial f(0.6,0.4)}{\partial x}$
et $\DS \frac{\partial f(0.2,0.3)}{\partial y}$.
\PDFgraph{15_var_mult_der/questbasic}
\label{15Q2}
\end{question}

\begin{question}
Soit la fonction $\DS f(x,y,z) = \frac{1}{1+xyz}$.  Si
$x= \cos(\theta+\phi)$, $y=\sin(\theta+\phi)$ et
$z=\cos(\phi)$, évaluez
$\DS \pdydx{f}{\theta}(\theta,\phi)$ au point
$(\theta, \phi) = (\pi, 0)$.
\label{15Q3}
\end{question}

\begin{question}[\eng]
Si $\DS w = f(x,y,z) = 2x^2 + y^2 + 2z^3$ où
$x=s+t^2$, $y=st$ et $z=s^2+t$, calculez
$\DS \pdydx{w}{s}$ et
$\DS \pdydx{w}{t}$ au point $(s,t) = (1,2)$.
\label{15Q4}
\end{question}
...
\begin{center}
\begin{tabular}{*{2}{l@{\hspace{0.5em}}l@{\hspace{3em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS \pdydx{f}{x}(\VEC{p})$ &
\subQ{b} & $\DS \pdydx{f}{y}(\VEC{p})$ &
\subQ{c} & $\DS \pdydxn{f}{x}{2}(\VEC{p})$ \\[0.8em]
\subQ{d} & $\DS \pdydxnm{f}{x}{y}{2}{}{}(\VEC{p})$ &
\subQ{e} & $\DS \pdydxn{f}{y}{2}(\VEC{p})$ & &
\end{tabular}
\end{center}
\label{15Q6}
\end{question}
...
\subQ{a} $\DS f(x,y)=x^2y+4y^2$ au point $(2,1)$ et dans la
direction du vecteur $(1,\sqrt{3})$. \\
\subQ{b} $\DS f(x,y,z) = xyz + \ln(x^2+y^2+z^2)$ au point
$(1,1,2)$ et dans la direction du vecteur
$\DS \VEC{v} = 2\ii + \jj - \kk$.\\
\subQ{c} $\DS f(x,y,z) = (x+y+z) e^{xyz}$ au point $(1,0,3)$
...
\subQ{a} $\DS f(x,y)=2x^2y^2+3xy+x$ au point $(1,1)$ pour
estimer $f(0.9,1.1)$.\\
\subQ{b} $\DS f(x,y) =  \ln\left(\frac{2x^2+5y^2}{7}\right)$
au point $(1,1)$ pour estimer $f(0.98,1.01)$.\\
\subQ{c} $\DS f(x,y) = (x^2-y^5)^{4/3}$ au point $(3,1)$
pour estimer $f(3.2,1.2)$.\\
\subQ{d} $\DS f(x,y)= 1- \frac{2x}{y}+3y-4xy^2+e^{3x}$ au
...
\subQ{d} & $\DS h(x,y)=x^3+y^3+3xy+\frac{1}{8}$ \\
...
d'équation $\DS x^2 + \frac{y^2}{4} + \frac{z^2}{9} = 1$.
\label{15Q24}
\end{question}
...
$\DS f(x,y) = \frac{1}{x} + \frac{1}{y}$ sous la contrainte
que $\DS \frac{1}{x^2} + \frac{1}{y^2} = 1$.
\label{15Q26}
\end{question}
...
\begin{center}
\begin{tabular}{*{1}{l@{\hspace{0.5em}}l@{\hspace{6em}}}l@{\hspace{0.5em}}l}
\subQ{a} & $\DS f(x,y) = \begin{pmatrix} x/y \\
  2xy \end{pmatrix}$ &
\subQ{b} & $\DS f(x,y) = \begin{pmatrix} x^2y+2xe^y \\
\DS \frac{x}{y}-3ye^{-x} \end{pmatrix}$
\end{tabular}
\end{center}
\label{15Q29}
\end{question}
