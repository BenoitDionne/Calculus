...
$\DS \VEC{x} = \begin{pmatrix} x_1 \\ x_2 \end{pmatrix}$.
Nous avons
\[
...
Par exemple, si
$\DS \VEC{x} = \begin{pmatrix} \sqrt{3} \\ 1 \end{pmatrix}$
et $\theta=\pi/3$, alors
\[
...
$\DS \frac{x_1^2}{3^2} + \frac{x_2^2}{2^2} = 1$.
\label{egg_ellipsoid}
\end{egg}
...
$\DS f(x,y) = \sum_{i=1}^n m_i\,x_i +b$ où les $m_i$ et
...
$\DS \left\{\VEC{x}_k\right\}_{k=1}^\infty$ de $\RR^n$
...
la suite $\DS \left\{\VEC{x}_k\right\}_{k=1}^\infty$.
\end{defn}
...
de la suite $\DS \left\{\VEC{x}_k\right\}_{k=1}^\infty$.
\end{defn}
...
\delta \} \subset V \; .
\]
$\DS B_\delta(\VEC{v})$ est la boule de rayon $\delta$
...
$\DS \left\{\VEC{x}_k\right\}_{k=1}^\infty$ de $D$ qui tend
...
$\DS \lim_{(x,y)\rightarrow (0,0)} \frac{\sin(x+y)}{x+y}$\ ?

Nous avons vu que
\[
\lim_{z\rightarrow 0} \frac{\sin(z)}{z} = 1 \; .
\]
Puisque $\DS \lim_{(x,y)\rightarrow (0,0)} x + y = 0$,
nous avons que
$\DS \lim_{(x,y)\rightarrow (0,0)} \frac{\sin(x+y)}{x+y} = 1$.

Pour être plus précis, si $\DS \{ (x_k,y_k) \}_{k=1}^\infty$
est une suite qui converge vers l'origine lorsque $k$ converge vers
plus l'infini, alors $\DS \{ x_k+y_k \}_{k=1}^\infty$ est
...
$\DS \{ (x_k,y_k) \}_{k=1}^\infty$ qui converge vers
...
Est-ce que $\DS \lim_{(x,y)\rightarrow (0,0)} f(x,y)$
...
obtenons la suite $\DS \{(x_k,y_k)\}_{k=0}^\infty$ qui converge
...
nous concluons que $\DS \lim_{(x,y)\rightarrow (0,0)} f(x,y)$
...
Notons que
$\DS \frac{xy-x}{x^2+y^2-2y+1} = \frac{x(y-1)}{x^2+(y-1)^2}$.
...
\[
f(x,y) = \begin{cases}
\DS \frac{x^3y^2}{x^2+y^2} & \quad \text{si} \quad (x,y)\neq (0,0) \\
...
\subQ{a} & $\DS f(x,y) = \ln(1-x^2-2y^2)$ &
\subQ{b} & $\DS f(x,y) = \sqrt{x^2-y^2}$
\end{tabular}
\end{center}
\label{14Q6}
\end{question}
...
\[
f(x,y) = \begin{cases}
\DS \frac{x\sqrt{y}}{x^2+y} & \text{si}\quad (x,y)\neq (0,0) \\
