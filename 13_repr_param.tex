\chapter[Représentation paramétriques des courbes \life \eng]{Représentations paramétriques des courbes\\ \life \eng}\label{chapReprParam}
...
soit $\DS \left\{ \phi_1, \phi_2, \ldots, \phi_n \right\}$
...
$-\pi/2<\theta<\pi/2$,
$\DS \lim_{\theta \to \pi/2} \sec(\theta) = 
\lim_{\theta \to \pi/2} \sec(\theta) =  +\infty$, 
$\DS \lim_{\theta \to -\pi/2} \tan(\theta) = +\infty$,
$\DS \lim_{\theta \to -\pi/2} \tan(\theta) =  -\infty$
...
&= \left(\DS \frac{\phi_1(t_0+h) - \phi_1(t_0)}{h},
\DS \frac{\phi_2(t_0+h) - \phi_2(t_0)}{h}, \ldots ,
\DS \frac{\phi_n(t_0+h) - \phi_n(t_0)}{h}\right)
\end{align*}
...
\left(\DS \lim_{h\rightarrow 0}\frac{\phi_1(t_0+h) - \phi_1(t_0)}{h},
\DS \lim_{h\rightarrow 0}\frac{\phi_2(t_0+h) - \phi_2(t_0)}{h},
 \ldots ,
\DS \lim_{h\rightarrow 0}\frac{\phi_n(t_0+h)
- \phi_n(t_0)}{h}\right) \\
...
$\DS \dydxn{y}{x}{2}(x(t))$ peut changer de signe seulement
lorsque cette dérivée seconde n'est pas définie; c'est-à-dire, lorsque
$t=0$ ou $2$.  Donc $\DS \dydxn{y}{x}{2}(x(t))$ ne change
...
Notons que $\DS \dydxn{y}{x}{2}(x(t))$ ne change pas de
signe lorsque $t$ est dans un des intervalles considérés ci-dessus
car $\DS \dydxn{y}{x}{2}(x(t)) =0$ seulement pour $t=0$,
$\pi$ ou $2\pi$; et $\DS \dydxn{y}{x}{2}(x(t))$ n'est pas
...
\end{align*}
Puisque $\DS \tan(\theta) = 2(x-1)$ et
$\DS \sec(\theta) = \sqrt{ 1 + 4(x-1)^2}$ pour
...
$\DS f'(x) = \frac{-2x}{1-x^2}$.  La longueur de la courbe
est
\begin{align*}
...
est $\DS 2\pi\,\left(\frac{R_1+R_2}{2}\right)\,L$.
...
Nous avons $\DS r = \sqrt{ (-1)^2 + (-\sqrt{3})^2} = 2$. Puisque
$x<0$, nous avons
\[
...
\hline
\rule{0pt}{1.8em} r & 3 &
\DS \frac{4+\sqrt{2}}{2} \approx 2.7071 & 2 &
\DS \frac{4-\sqrt{2}}{2} \approx 1.29289
& 1 & \DS \frac{4-\sqrt{2}}{2} & 2 &
\DS \frac{4+\sqrt{2}}{2} & 0
\end{array}
\]
Ces points apparaissent sur la courbe $\Gamma$ dans le dessin
...
$\DS f(r,\theta) = f\left(r,\frac{\pi}{2} -\theta\right) = 0$ (ou
$\DS f(r,\theta) = f\left(-r,-\frac{\pi}{2}-\theta\right)$).
\item La courbe $\Gamma$ est inchangée par une réflexion par rapport à
la droite $\theta = -\pi/4$ (i.e. $y=-x$) si 
$\DS f(r,\theta) = f\left(r,-\frac{\pi}{2} -\theta\right) = 0$
(ou
$\DS f(r,\theta) = f\left(-r,\frac{\pi}{2}-\theta\right)$).
...
$r(\theta_j^\ast)$ dont l'aire est $\DS \pi r^2(\theta_j^\ast)$.
