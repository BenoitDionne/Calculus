...
\begin{egg}
Le dessin du vecteur $\DS \VEC{q} = -2 \VEC{p}$, où
...
$\VEC{p}=(p_1,p_2)$, alors $\DS \VEC{p} = p_1 \ii + p_2 \jj$.
...
Si $\DS \ps{\VEC{p}}{\VEC{q}} = 0$, alors le cosinus entre
...
$\VEC{q}$ (et vice-versa) si $\DS \ps{\VEC{p}}{\VEC{q}} = 0$.
\end{defn}
...
\]
Puisque $\DS m_1 = \frac{p_2}{p_1}$ est la pente de la droite
$\ell_1$ et $\DS m_2 = \frac{q_2}{q_1}$ est celle de la droite
$\ell_2$, nous obtenons bien que $\DS m_1 = -\frac{1}{m_2}$.
\end{rmk}
...
Puisque $\DS \VEC{q} - \VEC{p} = (q_1-p_1,q_2-p_2)$,
nous avons que
\begin{align*}
\| \VEC{q} - \VEC{p} \|^2
&= (q_1 - p_1)^2 + (q_2-p_2)^2  \\
...
le vecteur $\VEC{p}$ est $\DS m_1 = \sqrt{3}$.  Comme la
...
points $\VEC{x}$ tels que le vecteur $\DS \VEC{x}-\VEC{p}$ est
...
Si $\DS \VEC{x}=(x_1,x_2,x_3)$, $\DS \VEC{p}=(p_1,p_2,p_3)$
et $\DS \VEC{m} = (m_1,m_2,m_3)$, la formule donnée à la
...
\end{equation}
où
$\DS  C_1 = \frac{k_{\VEC{n}} m_1 - n_1 k_{\VEC{m}}}{n_2 m_1 - n_1 m_2}$ et
$\DS  C_2 = - \frac{m_1 n_3 - n_1 m_3}{n_2 m_1 - n_1 m_2}$.
...
$\DS  C_4 = - \frac{m_2 n_3 - n_2 m_3}{n_2 m_1 - n_1 m_2}$.
