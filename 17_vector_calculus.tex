...
\begin{egg}
Pour tracer le champ de vecteurs donné par $\DS
...
$\DS \int_C xy \dx{x} - x \dx{y}$ le long de la courbe $C$
...
$\DS \int_C = - \int_{-C}$.  Une représentation
...
\]
Donc $\DS \int_{\Gamma_1} F \cdot \dx{\VEC{s}} =
\int_{\Gamma_2} F \cdot \dx{\VEC{s}}$ si et seulement si
$\DS \int_{\Gamma_3} F \cdot \dx{\VEC{s}} = 0$.
\end{rmk}
...
$\DS \int_\Gamma F \cdot \dx{\VEC{s}}$ est
...
\]
nous avons que $\DS \pdydx{F_2}{x} = \pdydx{F_1}{y}$ et donc
...
Puisque $\DS \pdydx{\phi}{t}(t,\VEC{x}) = F(\phi(t,\VEC{x}))$,
nous avons
\[
...
$\DS \dydx{\VEC{b}}{t}(t,\VEC{x}_0)$ et
$\DS \dydx{\VEC{c}}{t}(t,\VEC{x}_0)$.
...
\end{equation}
avec un résultat semblable pour
$\DS \dydx{\VEC{b}}{t}(0,\VEC{x}_0)$ et
$\DS \dydx{\VEC{c}}{t}(0,\VEC{x}_0)$.  Le volume de $P_t$ est
\[
...
&= \frac{1}{2}
\begin{pmatrix}
0 & \DS \pdydx{F_1}{y} - \pdydx{F_2}{x} &
\DS \pdydx{F_1}{z} - \pdydx{F_3}{x} \\[1em]
\DS \pdydx{F_2}{x} - \pdydx{F_1}{y} & 0 &
\DS \pdydx{F_2}{z} - \pdydx{F_3}{y} \\[1em]
\DS \pdydx{F_3}{x} - \pdydx{F_1}{z} &
\DS \pdydx{F_3}{y} - \pdydx{F_2}{z} & 0
\end{pmatrix} \\
&= \frac{1}{2}
\begin{pmatrix}
0 & \DS -\left(\curl F\right)_3 &
\DS \left(\curl F\right)_2 \\
\DS \left(\curl F\right)_3 & 0 &
\DS -\left(\curl F\right)_1 \\
\DS -\left(\curl F\right)_2 & 
\DS \left(\curl F\right)_1 & 0
\end{pmatrix}
\end{align*}
...
l'axe de rotation est l'axe des $z$.  Supposons que
...
rotationnel pour une objet qui se déplace sous l'effet d'un
flot.}{diff_geom_CURL}

Soit $\VEC{p}$ un point de $S$.
Nous avons que $\VEC{p} = P(\VEC{p}) + \VEC{r}$ où $P(\VEC{p})$ est la
projection orthogonal de $\VEC{p}$ sur l'axe des $z$ et $\VEC{r}$ est
un vecteur perpendiculaire à l'axe de rotation (l'axe des $z$) qui
va de l'axe de rotation à l'extrémité de $\VEC{p}$.

Le vecteur $\VEC{v} = \VEC{w} \times \VEC{r}$ est perpendiculaire au
...
Puisque $\VEC{w} = (0,0,\omega)$, si $\VEC{p} = (x,y,z)$, nous avons que
\[
\VEC{v} = \VEC{w} \times \VEC{r} = \VEC{w} \times (\VEC{p} - P(\VEC{p}))
= \VEC{w} \times \VEC{p} = (-\omega y, \omega x, 0)
\]
puisque $P(\VEC{p})$ est parallèle à $\VEC{w}$.  Ainsi, on obtient que
$\curl \VEC{v}  = 2\, \omega\, \kk$.
...
En conclusion, si $\curl \VEC{v} = \VEC{0}$, alors $\omega =0$.
L'objet ne tourne pas sur lui même.  Si on considère un objet qui se
déplace le long d'un flot associé à un champ de vecteur $F$, alors
$\VEC{v} = F(\VEC{p})$.  Donc, si $\curl F = \VEC{0}$, alors l'objet
...
= \begin{vmatrix}
\ii & \jj & \kk \\
\DS \pdydx{\rho_1}{u_1} & \DS \pdydx{\rho_2}{u_1} &
\DS \pdydx{\rho_3}{u_1} \\[0.8em]
\DS \pdydx{\rho_1}{u_2} & \DS \pdydx{\rho_2}{u_2} &
\DS \pdydx{\rho_3}{u_2} \end{vmatrix}
\]
...
$\DS \iint_S \dx{S}$ est l'aire de la surface $S$.
\end{prop}
...
$\DS \pdydx{\rho}{u_1}$
et $\DS \pdydx{\rho}{u_2}$ prennent la forme
$\DS \pdydx{\rho}{u}
...
$\DS \pdydx{\rho}{u}(u_i,v_j) \Delta u_i$ et
$\DS \pdydx{\rho}{v}(u_i,v_j) \Delta v_j$
...
\Delta u_i \ \Delta v_j \ .
\]
Donc $\DS \iint_S \dx{\VEC{S}}$ est l'aire de la surface $S$.
\label{AreaReprRMK}
\end{rmk}
...
$\DS \frac{\partial(x,y)}{\partial(r,\theta)} = r $.  De
...
\frac{1}{\DS \left\| \pdydx{\rho}{u_1}(\VEC{u}) \times
...
$\DS \pdydx{\rho}{u_1}(\VEC{u}) \times \pdydx{\rho}{u_2}(\VEC{u})$
...
$\DS \pdydx{\rho}{u} \times \pdydx{\rho}{v}$ pointe
...
\DS \pdydx{\rho_1}{u}(u_i,v_j) \Delta u_i &
\DS \pdydx{\rho_2}{u}(u_i,v_j) \Delta u_i &
\DS \pdydx{\rho_3}{u}(u_i,v_j) \Delta u_i \\[1em]
\DS \pdydx{\rho_1}{v}(u_i,v_j) \Delta v_j &
\DS \pdydx{\rho_2}{v}(u_i,v_j) \Delta v_j &
\DS \pdydx{\rho_3}{v}(u_i,v_j) \Delta v_j
\end{pmatrix}
\end{align*}
...
$\DS \pdydx{\rho}{u}(u_i,v_j)$ et
$\DS \pdydx{\rho}{v}(u_i,v_j)$ sont tangents à la surface
...
\Delta u_i \ \Delta v_j
\end{align*}
Donc $\DS \iint_S F \cdot \dx{\VEC{S}}$ est le
...
\end{align*}
Puisque $z>0$, le vecteur $\DS \VEC{m}$
...
\VEC{n}(x,y) &=
\frac{\epsilon}
{\DS \left\| \left(-\pdydx{g}{x}(x,y) , -\pdydx{g}{y}(x,y) , 1\right)
\right\|} \; \left(-\pdydx{g}{x} , -\pdydx{g}{y}, 1\right) \\
&= \frac{\epsilon}{\DS \sqrt{ \left(\pdydx{g}{x}(x,y)\right)^2 +
...
$\DS \int_C F \cdot \dx{\VEC{s}}$.  Nous ne ferons pas cela.
...
= \left| \det \begin{pmatrix} \DS \pdydx{x}{r} &
\DS \pdydx{y}{r} \\[0.7em]
\DS \pdydx{y}{r} & \DS \pdydx{y}{\theta}
\end{pmatrix} \right|
...
$\DS \int_{\partial S} F \cdot \dx{\VEC{s}}$ que nous
...
\]
C'est-à-dire que $\DS \int_{\partial S} x \dx{y}$ est l'aire
...
\]
Encore là, $\DS -\int_\Gamma y \dx{x}$ est l'aire de la
...
\begin{egg}
Calculons $\DS \int_C F \cdot \dx{\VEC{s}}$ où
...
Nous pourrions calculer $\DS \int_C F \cdot \dx{\VEC{s}}$
...
$\DS \iint_S \curl  F \cdot \dx{\VEC{S}}$ où
...
vecteur $\DS \VEC{m}$ est un vecteur normal à la surface $S$
...
\[
  ...
  \int_{C_r} F\cdot \dx{\VEC{s}} \ .
\]
Puisque $\DS \lim_{r\to 0} \VEC{x}_r = \VEC{x}_0$, nous obtenons
\[
...
$\DS \int_{C_r} F\cdot \VEC{t} \dx{s}$ représente le
déplacement le long de $C_r$.  Donc, pour $r$ très petit,
$\DS \frac{1}{\pi r^2} \int_{C_r} F\cdot \VEC{t} \dx{s}$
...
maximale pour
$\DS \VEC{v} = \frac{1}{\|\curl F(\VEC{x}_0)\|}\; \curl F(\VEC{x}_0)$.
Dans ce cas,
...
\left| \det \begin{pmatrix}
  \DS \pdydx{x}{\theta} & \DS \pdydx{x}{\phi}
  & \DS \pdydx{x}{r} \\[0.8em]
  \DS \pdydx{y}{\theta} & \DS \pdydx{y}{\phi}
  & \DS \pdydx{y}{r} \\[0.8em]
  \DS \pdydx{z}{\theta} & \DS \pdydx{z}{\phi}
  & \DS \pdydx{z}{r}
\end{pmatrix} \right| \\
&= 
\left| \det \begin{pmatrix}
...
\begin{question}
Évaluez l'intégrale $\DS \int_C F \cdot \dx{\VEC{s}}$ où
...
\begin{question}
Évaluez l'intégrale
$\DS \int_C x \sqrt{y} \dx{x} + 2xy \dx{y}$ où $C$
...
calculez $\DS \int_C F \cdot \dx{\VEC{s}}$ le long de
la courbe $C$ donnée.\\
...
\begin{question}
Évaluez l'intégrale $\DS \iint_S xy \dx{S}$ où $S$
...
\begin{question}
Évaluez l'intégrale $\DS \iint_S (x^2+y^2)z \dx{S}$ où $S$
...
\begin{question}
Calculez l'intégrale $\DS \iint_S F \cdot \dx{\VEC{S}}$
...
$\DS \int_C(2xy -x^2)\dx{x} + (x + y^2)\dx{y}$
...
\begin{question}
Évaluez l'intégrale
$\DS \int_C(y+e^{x^2})\dx{x} + (2x + \cos(y^2))\dx{y}$
...
$\DS \int_C F \cdot \dx{\VEC{s}}$ où le champ de
...
$\DS \iint_S F \cdot \dx{\VEC{S}}$ à l'aide du
