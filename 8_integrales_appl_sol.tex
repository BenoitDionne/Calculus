\nonumsection{Chapter~\ref{ChapApplInt} : Applications de l'intégrale}

\nonumsubsection{Aire entre deux courbes}
...
$\DS \frac{3}{2} + x > \frac{x^2}{2}$ pour $-1 < x < 3$,
...
$\DS y=7x -x^2$ et la droite $\DS y=x$ se
...
$\DS x=y^2$ et la droite $\DS x-y=2$ se
...
$\DS y=\frac{x}{1+x}$ et la droite $\DS y=\frac{x}{2}$ se
coupent sont les solutions de 
$\DS \frac{x}{1+x} = \frac{x}{2}$.  Nous avons la solution $x=0$.
Si $x\neq 0$, nous pouvons diviser les deux côtés de l'équation par $x$ pour
obtenir $\DS \frac{1}{1+x} = \frac{1}{2}$.  Une seconde
...
\subQ{b}
Puisque $\DS \lim_{T\to \infty} (2+3T)^{1/4} = +\infty$, nous avons que
$\DS \lim_{T\to \infty} n(T) = +\infty$.  Cette population de
...
\nonumsubsection{Valeur moyenne d'une fonction}
...
$\DS M = \frac{T}{24} = \frac{11650.5}{24} = 485.4375$ J/hr.
}
...
\nonumsubsection{Volume d'un objet}
...
$\DS A(x) = \pi \left(\frac{e^{2x}-e^x}{2}\right)^2$.
Le volume est donc
\begin{align*}
V &= \int_0^1 A(x) \dx{x}
...
$\DS 9 \sqrt{1 - \frac{(z_i^\ast)^2}{15^2}}$.  Le volume du
...
tranche est
$\DS A(x) = \frac{1}{2} \pi (\sqrt{x})^2 = \frac{1}{2} \pi x$.
Ainsi,
\[
...
L'aire d'un tranche est $\DS A(x) = (3x+x^2)^2$.
...
\nonumsubsection{Masse d'un objet}

\nonumsubsection{Travail}
...
$\DS -9,800 \pi \int_{-5}^{-2} x (9 - (-2-x)^2)\dx{x}$.
...
$\DS -19600  \int_{-6}^{-5.9} x \sqrt{0.0025 - (x + 5.95)^2} \dx{x}$,
nous obtenons
\[
...
Donc $\DS F = 19600 \times 5.95 \times \frac{0.0025}{2}
=145.775\pi$ N.
}

\nonumsubsection{Centre de masse}
...
\nonumsubsection{Applications à l'économie}

\nonumsubsection{Test de l'intégrale}

\compileSOL{\SOLUb}{\ref{8Q36}}{
\subQ{a}
Nous avons la séries $\DS \sum_{n=1}^\infty a_n$ où $a_n = g(n)$
pour $\DS g(x) = x e^{-2x^2}$.
...
= \frac{e^{-2}}{4} \ .
\]
Donc, puisque l'intégrale $\DS \int_1^\infty x e^{-2x^2} \dx{x}$
converge, la série converge.
}

\compileSOL{\SOLUb}{\ref{8Q38}}{
Nous avons la séries $\DS \sum_{n=1}^\infty a_n$ où $a_n = f(n)$
pour $\DS f(x) = \frac{1}{x^{8/3}}$.

Puisque $f(x)$ est une fonction positive décroissante qui converge vers
$0$ lorsque $x\to \infty$ et
$\DS \int_1^\infty f(x) \dx{x} = \frac{3}{5}$, il découle
du test de l'intégrale que la séries
$\DS \sum_{n=1}^\infty \frac{1}{n^{8/3}}$
converge car $\DS \int_1^\infty f(x) \dx{x}$ converge.
...
$\DS \frac{3}{5N^{5/3}} < 10^{-3}$.  Donc, il faut avoir
\[
