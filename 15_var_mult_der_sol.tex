\nonumsection{Chapter~\ref{chapDerFonctMult} : Dérivée de fonctions de plusieurs variables}

\nonumsubsection{Dérivées partielles}

\compileSOL{\SOLUb}{\ref{15Q1}}{
\subQ{a}
$\DS f_x(x,y,z,t) = y^2z^3t^4$ car $y$, $z$ et $t$ se
comportent comme des constantes.

$\DS f_y(x,y,z,t) = 2xyz^3t^4$ car $x$, $z$ et $t$ se
comportent comme des constantes.

$\DS f_z(x,y,z,t) = 3xy^2z^2t^4$ car $x$, $y$ et $t$ se
comportent comme des constantes.

$\DS f_t(x,y,z,t) = 4xy^2z^3t^3$ car $x$, $y$ et $z$ se
comportent comme des constantes.

\subQ{b}
$\DS f_x(x,y) = \frac{2 (x-y) - (2x+y)}{(x-y)^2} =
\frac{-3y}{(x-y)^2}$ car $y$ se comporte comme une constante.

$\DS f_y(x,y) = \frac{(x-y) + (2x+y)}{(x-y)^2} =
\frac{3x}{(x-y)^2}$ car $x$ se comporte comme une constante.

\subQ{c}
$\DS \pdydx{h}{x} = \pdfdx{\big(f(x)g(y)\big)}{x} = f'(x)g(y)$ car
$y$ (et donc $g(y)$) se comporte comme une constante.

$\DS \pdydx{h}{y} = \pdfdx{\big(f(x)g(y)\big)}{y} = f(x)g'(y)$ car
$x$ (et donc $f(x)$) se comporte comme une constante.

\subQ{d}
$\DS \pdydx{h}{x} = \pdfdx{ f(xy) }{x} = f'(xy)
\pdfdx{\big(xy\big)}{x} = f'(xy)\, y$ car $y$ se comporte comme une constante.

$\DS \pdydx{h}{y} = \pdfdx{ f(xy) }{y} = f'(xy)
...
\nonumsubsection{Plan tangent à une surface}
...
\nonumsubsection{Dérivées selon un direction donnée}
...
\nonumsubsection{Propriétés du gradient}

\compileSOL{\SOLUb}{\ref{15Q14}}{
La surface est décrite par l'équation
$\DS F(x,y,z) = x^2 -4xy + y^2 + z^2 = 2$.
...
Donc $\DS x_0 = \frac{3}{2}\lambda$,
$\DS y_0 = -\frac{1}{4}\lambda$ et
$\DS z_0 = \frac{1}{2}\lambda$.
...
\]
Donc $\DS \lambda = \pm \frac{2\sqrt{2}}{5}$.
...
\nonumsubsection{Points critiques et valeurs extrêmes}
...
$\DS \pdydx{f}{x} = y -2xy -y^2$ et
$\DS \pdydx{f}{y} = x -x^2 -2xy$, nous obtenons
\[
H(x,y) = \begin{pmatrix}
\DS \pdydxn{f}{x}{2}(x,y) &
...
Puisque $\DS \det H(1,2)
= \det \begin{pmatrix} 6 & 0 \\ 0 & 12 \end{pmatrix} = 72 > 0$
et $\DS \pdydxn{f}{x}{2}(1,2) = 6 > 0$, il y a un 
...
Puisque $\DS \det H(1,-1)
= \det \begin{pmatrix} 6 & 3 \\ 3 & 6 \end{pmatrix} = 27 > 0$ et
$\DS \pdydxn{f}{x}{2}(1,-1) = 6 >0$, il y a un
...
$\DS \pdydxn{f}{x}{2}(0,0) = 10 >0$, il y a un
minimum local au point $(0,0)$.

Puisque $\DS \det H(0,0)
= \det \begin{pmatrix} 10 & 0 \\ 0 & 2 \end{pmatrix} = 40/3 > 0$ et
$\DS \pdydxn{f}{x}{2}(-5/3,0) = -10 < 0$, il y a un maximum
...
\nonumsubsection{Multiplicateurs de Lagrange}
...
que $\DS g(x,y,z) = x^2 + \frac{y^2}{4} + \frac{z^2}{9} - 1 = 0$.
...
$\DS x^2 + \frac{y^2}{4} + \frac{z^2}{9} = 1$, nous obtenons
$3x^2 = 1$.  Donc $x = 1/\sqrt{3}$.  Cela donne le point
$\DS \left( \frac{1}{\sqrt{3}}, 
...
Le volume maximal de $f$ est donc $\DS \frac{16}{\sqrt{3}}$
aux points $\DS \left( \frac{1}{\sqrt{3}}, 
\frac{2}{\sqrt{3}}, \frac{3}{\sqrt{3}} \right)$,
$\DS \left( -\frac{1}{\sqrt{3}}, 
-\frac{2}{\sqrt{3}}, \frac{3}{\sqrt{3}} \right)$
$\DS -\left( \frac{1}{\sqrt{3}}, 
...
$\DS \left( \frac{1}{\sqrt{3}}, 
  -\frac{2}{\sqrt{3}}, -\frac{3}{\sqrt{3}} \right)$.
Le minimum absolu de $f$ est $\DS -\frac{16}{\sqrt{3}}$
...
$\DS g(x,y) = \frac{1}{x^2} + \frac{1}{y^2} - 1 = 0$ et qui
donnent le maximum de $\DS f(x,y) = \frac{1}{x} + \frac{1}{y}$.
...
$g(x,y) = 0$, nous obtenons $\DS \lambda^2 = 1/2$.
...
par $\DS E \cap \{(x,y) : \|(x,y)\| \leq M \}$ pour $M$
...
\nonumsubsection{Dérivées des fonctions de $\mathbf{\RR^n}$ dans
 $\mathbf{\RR^m}$}
...
Df(x,y) =
\begin{pmatrix}
\DS \pdydx{f_1}{x}(x,y) & \DS \pdydx{f_1}{y}(x,y) \\[1em]
\DS \pdydx{f_2}{x}(x,y) & \DS \pdydx{f_2}{y}(x,y)
\end{pmatrix}
\]
