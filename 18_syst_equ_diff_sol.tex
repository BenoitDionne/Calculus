\nonumsection{Chapter~\ref{ChapSystEquDiff} : Systèmes d'équations
différentielles}

\nonumsubsection{Systèmes d'équations différentielles linéaires}
...
\hline
\rule{0em}{1.4em} $\lambda_1 = -5$ & $\DS \VEC{v}_1 =
\begin{pmatrix} -2 & 1 \end{pmatrix}^\top$ \\[0.5em]
$\lambda_2 = 7$ & $\DS \VEC{v}_2 =
...
$\DS \begin{pmatrix} 0 & 1 \end{pmatrix}^\top$ à $t=0$, il
...
$\DS \alpha = \beta = 1/2$.  La solution qui satisfait la
condition initiale est
\[
\VEC{x} = \begin{pmatrix}
- e^{-5 t} + e^{7t} \\
\DS \frac{1}{2} e^{-5 t} + \frac{1}{2} e^{7t}
\end{pmatrix} \ .
\]
...
\rule{0em}{1.4em}
$\lambda = -1$ &
$\DS \VEC{v} =\begin{pmatrix} 1 & -1 \end{pmatrix}^\top$
\end{tabular}
\end{center}
Il faut donc trouver un vecteur $\VEC{u}$ tel que $(A+\Id)\VEC{u} = \VEC{v}$.
Si nous posons
$\DS \VEC{u} = \begin{pmatrix} a & b \end{pmatrix}^\top$, il
...
$\DS \VEC{u} = \begin{pmatrix} 1 & 0 \end{pmatrix}^\top$.
...
\end{align*}
Pour satisfaire la condition initiale
$\DS \begin{pmatrix} 0 \\ 1 \end{pmatrix}$ à $t=0$, il faut
...
\diff \VEC{f}(x,y) =
\begin{pmatrix}
\DS \pdydx{f_1}{x}(x,y) & \DS \pdydx{f_1}{y}(x,y) \\[0.8em]
...
\[
  ...
  \end{pmatrix} = \VEC{f}(x,y)
\]
où $\DS f_1(x,y) = x(1-x)\left(x-\frac{1}{2}\right) - \frac{xy}{8}$
...
\end{pmatrix}
= \begin{pmatrix}
\DS -3x^2 + 3x - \frac{1}{2} - \frac{1}{8}\,y &
\DS  -\frac{1}{8}\,x \\
y & -d+ x
\end{pmatrix} \ .
\]
...
\diff \VEC{f}(\VEC{p}) =
\begin{pmatrix}
\DS -3d^2 + 3d - \frac{1}{2} - (1-d)\left(d-\frac{1}{2}\right) &
\DS  -\frac{1}{8}\,d \\
\DS 8(1-d)\left(d-\frac{1}{2}\right) & 0
\end{pmatrix}
= \begin{pmatrix}
\DS -2d^2 + \frac{3d}{2}& \DS -\frac{1}{8}\,d \\
-8d^2 +12d -4 & 0
\end{pmatrix} \ .
\]
...
\[
  \begin{pmatrix} x' \\ y' \end{pmatrix} =
  \begin{pmatrix}
\DS -2d^2 +\frac{3 d}{2} & \DS -\frac{d}{8} \\
