\nonumsection{Chapter~\ref{chapFonctPlusVar} : Fonctions de plusieurs variables}

\nonumsubsection{Fonctions de plusieurs variables}
...
$\DS \frac{-5-5}{6-1} = -2$.  Or, si $x=2$ dans
...
le diagramme de courbes de niveau dans l'énoncé de la question.
C'est donc une droite de pente $\DS \frac{25+5}{10-0} = 3$.
...
$\DS f(x,y) = \sqrt{x^2-y^2}$ dans la figure ci-dessous.
\MATHgraph{14_var_mult_fonct/level2}{8cm}
}

\compileSOL{\SOLUb}{\ref{14Q7}}{
Les courbes de niveau de $f$ sont données par
$\DS \frac{1}{1+x^2+y^2} = C \in ]0,1]$.  Ainsi,
$\DS x^2 + y^2 = D$ où
$\DS D = \frac{1}{C} - 1 \in [0, \infty[$.
...
= \begin{cases}
\DS \frac{\sqrt{m}\,x^2}{(1+m)x^2} = \frac{\sqrt{m}}{1+m} &
\text{si } x>0 \\[2ex]
\DS \frac{-\sqrt{m}\,x^2}{(1+m)x^2} = \frac{-\sqrt{m}}{1+m}
& \text{si } x<0 \\
\end{cases}
...
$\DS \lim_{(x,y)\rightarrow(0,0)} f(x,y)$ n'existe donc pas.  Par
